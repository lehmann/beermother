...
export function sanitizeXmlText(e) {
    const t = String(e || "").replace(/&#(x[0-9a-fA-F]+|\d+);/g, (i, r) => {
        const o = r[0] ? .toLowerCase() === "x" ? Number.parseInt(r.slice(1), 16) : Number.parseInt(r, 10);
        return Number.isFinite(o) && isValidXmlCodePoint(o) ? i : ""
    });
    return Array.from(t).filter(i => isValidXmlCodePoint(i.codePointAt(0))).join("")
}
...
export function parseBeerXml(e) {
    if (!e || !e.trim()) throw new Error(b("Abra uma receita pela comunidade antes de iniciar."));
    const t = new DOMParser().parseFromString(sanitizeXmlText(e), "application/xml");
    if (t.querySelector("parsererror")) throw new Error(b("Receita inv\xE1lida. Confira o arquivo publicado."));
    ...
    const o = first(r, "EQUIPMENT"),
        ...
        Y = Number.isFinite(O) && O > 0 ? C(L, _, p) : Number.isFinite(h) ? h : C(L, _, p),
        ...
        x = children(M, "FERMENTABLE").map((n, s) => {
            ...
            return {
                ...
                type: c,
                use: rt ? "Fermenta\xE7\xE3o" : c === "Gr\xE3o" || c === "Adjunto" ? "Mostura" : "Fervura",
        ...
        Q = x.filter(n => n.use === "Mostura").reduce((n, s) => n + a(s.amountKg), 0),
...
export function importedColorEbc(e) {
    const t = textOf(e, "EST_COLOR", "") || textOf(e, "COLOR", ""),
        i = f(t);
    return !Number.isFinite(i) || i <= 0 ? 0 : /ebc/i.test(t) ? u(i, 1) : /srm/i.test(t) ? u(i * 1.97, 1) : /lov|°?\s*l\b/i.test(t) ? u(Math.max(0, i * 1.3546 - .76) * 1.97, 1) : u(i * 1.97, 1)
}
...
export function importedDirectEvaporationLh(e) {
    return firstOptionalNumber(e, ["BOIL_OFF_PER_HOUR", "BOIL_OFF_PER_HR", "BOIL_OFF_HOUR", "BOIL_OFF_RATE", "BOIL_OFF_LH", "BOIL_OFF_L_PER_HOUR", "EVAPORATION_LH", "EVAPORATION_L_PER_HOUR"])
}
export function importedEvaporationLh(e, t = {}) {
    ...
    const E = Math.max(.01, a(t.boilTimeMin, 60) / 60),
        M = (a(t.batchVolumeL) + a(t.trubLossL)) / ot;
    if (Number.isFinite(r) && r > 0) return u(d(r, M, t.boilTimeMin), 2);
    const T = Math.max(0, o - M) / E;
    return T > 0 ? u(T, 2) : u(d(l.evaporationPct, R(l.targetVolumeL, l.targetVolumeL * l.trubLossPct), t.boilTimeMin), 2)
}
export function importedMashEfficiencyPct(e, t = {}) {
    const i = optionalNumber(e, "MASH_EFFICIENCY");
    return Number.isFinite(i) ? i : t.hasEquipmentEfficiency ? ct(t.equipmentEfficiencyPct, t.batchVolumeL, t.trubLossL) : a(t.equipmentEfficiencyPct, 70)
}

function ct(e, t, i) {
    const r = Math.max(.01, a(t)),
        o = r + Math.max(0, a(i));
    return u(a(e, l.mashEfficiencyPct) * o / r, 1)
}
export function fermentationProfile(e) {
    return [{
        name: "Inocula\xE7\xE3o",
        temp: "PRIMARY_TEMP",
        days: "PRIMARY_AGE"
    }, {
        name: "Prim\xE1ria",
        ...
        return !Number.isFinite(i) && !Number.isFinite(r) ? null : {
            name: t.temp === "AGE_TEMP" && Number.isFinite(i) && i > 12 ? "Matura\xE7\xE3o" : t.name,
            temperatureC: i,
            days: r
        }
    }).filter(Boolean)
...
export function miscAmountInfo(e) {
    ...
    if (Number.isFinite(i) && r) return {
        amount: i,
        unit: r
    };
    const o = number(e, "AMOUNT", 0);
    return /true|1|yes/i.test(textOf(e, "AMOUNT_IS_WEIGHT", "")) ? o && Math.abs(o) < 1 ? {
        amount: o * 1e3,
        unit: "g"
    } : {
        amount: o,
...
export function ingredientUnitFromText(e) {
    ...
    return /\bmg\b/.test(t) ? "mg" : /\bkg\b/.test(t) ? "kg" : /\bg\b/.test(t) ? "g" : /\bml\b/.test(t) ? "mL" : /(^|[\d\s])l\b/.test(t) ? "L" : ""
}
...
export function yeastUnitFromText(e) {
    const t = String(e || "").toLowerCase();
    return /pacote/.test(t) ? "pacote" : /sach[eê]|sachet/.test(t) ? "sach\xEA" : /\bmg\b/.test(t) ? "mg" : /\bkg\b/.test(t) ? "kg" : /\bg\b/.test(t) ? "g" : /\bml\b/.test(t) ? "mL" : /(^|[\d\s])l\b/.test(t) ? "L" : ""
}
export function normalizeFermentableType(e) {
    const t = String(e).toLowerCase();
    return t.includes("sugar") ? "A\xE7\xFAcar" : t.includes("extract") ? "Extrato" : t.includes("adjunct") ? "Adjunto" : "Gr\xE3o"
}
...
export function normalizeHopUse(e) {
    const t = String(e).toLowerCase();
    return t.includes("mash") ? "Mostura" : t.includes("dry") ? "Dry hop" : t.includes("first") ? "First wort" : t.includes("whirlpool") ? "Whirlpool" : t.includes("aroma") || t.includes("hopstand") ? "Hopstand" : "Fervura"
}
export function normalizeMiscUse(e) {
    const t = String(e || "").toLowerCase();
    return t.includes("mash") ? "Mostura" : t.includes("first") ? "First wort" : t.includes("whirlpool") ? "Whirlpool" : t.includes("aroma") || t.includes("hopstand") ? "Hopstand" : t.includes("dry") ? "Dry hop" : t.includes("primary") || t.includes("secondary") || t.includes("ferment") ? "Fermenta\xE7\xE3o" : t.includes("bottl") || t.includes("packag") ? "Envase" : "Fervura"
}
export function normalizeMiscType(e) {
    const t = String(e || "").toLowerCase();
    return t.includes("spice") ? "Especiaria" : t.includes("fining") ? "Clarificante" : t.includes("herb") ? "Erva" : t.includes("flavor") ? "Aroma" : "Insumo"
}
