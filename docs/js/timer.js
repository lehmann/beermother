...
import { app as s } from "./state.js";
import { toast as y } from "./ui.js";
import { t as n, tEngine as l } from "./i18n.js";
export const timer = {
  state: null,
  interval: null,
  openContext: "",
...
export function startTimer(t, e) {
  ...
  ((timer.state = {
    ...
    durationMs: a,
    stageRemainingMs: Number.isFinite(Number(e.stageRemainingMin))
      ? e.stageRemainingMin * 60 * 1e3
      : a,
    targetStageRemainingMs: Number.isFinite(Number(e.targetStageRemainingMin))
      ? e.targetStageRemainingMin * 60 * 1e3
      : 0,
    boilRemainingMs: Number.isFinite(Number(e.boilRemainingMin))
      ? e.boilRemainingMin * 60 * 1e3
      : 0,
    targetBoilRemainingMs: Number.isFinite(Number(e.targetBoilRemainingMin))
      ? e.targetBoilRemainingMin * 60 * 1e3
      : 0,
    startedAt: Date.now(),
    endsAt: Date.now() + a,
    pausedRemainingMs: 0,
...
export function currentTimerItemIndex(t) {
  ...
  const a = timer.state.phase || "boil",
    i =
      r(timer.state.targetStageRemainingMs, timer.state.targetBoilRemainingMs) /
      6e4;
  return t.findIndex(
    (o) =>
      o.phase === a &&
      Math.abs(r(o.targetStageRemainingMin, o.targetBoilRemainingMin) - i) <
        0.02,
  );
}
export function completeTimerSegment() {
  timer.state &&
    ...
    (timer.state.pausedRemainingMs = 0),
    (timer.state.endsAt = Date.now()),
    c());
}
...
export function startFirstTimer(t) {
  const e = timerItemsForContext(t),
    a = ["boil", "mash"].includes(t)
      ? e[0]
      : e.find((i) => r(i.durationMin) > 0);
  a && startTimer(t, a);
}
export function toggleTimerPause(t) {
  ...
  timer.state.status === "running"
    ...
    : timer.state.status === "paused"
      ? resumeTimer()
      : timer.state.status === "done" && resumeFromHold(t);
}
export function resumeFromHold(t) {
  const e = timerItemsForContext(t);
  if (!e.length) return;
  const a =
    timer.state && timer.state.context === t && timer.state.status === "done"
      ? timer.state
      : null;
...
function x(t) {
  const e =
    t?.eventType === "mash-end"
      ? n("Mostura finalizada.")
      : t?.eventType === "hopstand-end"
        ? n("Hopstand finalizado.")
        : t?.eventType === "boil-end"
          ? n("Fervura finalizada.")
          : "";
...
export function timerRemainingMs() {
  return timer.state
    ? timer.state.status === "paused"
      ? Math.max(0, r(timer.state.pausedRemainingMs))
      : Math.max(0, r(timer.state.endsAt) - Date.now())
    : 0;
}
export function activeTimerStageRemainingMs() {
  ...
  return Math.max(
    0,
    r(timer.state.stageRemainingMs, timer.state.boilRemainingMs) - t,
  );
}
...
function k() {
  if (u)
    try {
      ...
      [0, 0.28, 0.56].forEach((e, a) => {
        const i = u.createOscillator(),
          o = u.createGain();
        ((i.type = "sine"),
          (i.frequency.value = a === 2 ? 1175 : 880),
          o.gain.setValueAtTime(1e-4, t + e),
          o.gain.exponentialRampToValueAtTime(0.32, t + e + 0.02),
          o.gain.exponentialRampToValueAtTime(1e-4, t + e + 0.22),
          i.connect(o).connect(u.destination),
          i.start(t + e),
          i.stop(t + e + 0.24));
      });
    } catch {}
...
async function p() {
  ...
  try {
    (t &&
      ...
      ((d = await navigator.wakeLock.request("screen")),
      d.addEventListener?.("release", () => {
        d = null;
      })),
...
export function startTimerTicker() {
  timer.interval ||
    (timer.interval = window.setInterval(() => {
      (c(),
        (!timer.state || timer.state.status !== "running") &&
          (window.clearInterval(timer.interval), (timer.interval = null)));
    }, 1e3));
}
function c() {
    ...
    if (t.context === "boil") {
      if (t.eventType === "addition") {
        ...
        (recordTimerEvent({
          ...
          detail: [
            t.eventTimeLabel || "-",
            r(t.eventCount) ? l(g(r(t.eventCount))) : "",
            i,
          ]
            .filter(Boolean)
            .join(" \xB7 "),
...
export function recordTimerEvent(t = {}) {
  const e = s.session;
  e &&
    ((e.timerEvents = e.timerEvents || []),
    !(t.key && e.timerEvents.some((a) => a.key === t.key)) &&
      (e.timerEvents.push({
...
export function hasTimerEvent(t) {
  return !!(
    s.session && (s.session.timerEvents || []).some((e) => e.key === t)
  );
}
function w(t = []) {
  const e = s.session;
  ...
  (t.forEach((i) => {
    ...
    e.additionChecks[o] ||
      ((e.additionChecks[o] = new Date().toISOString()), (a = !0));
  }),
    a && s.requestRender());
}
export function timerAdditionsDetail(t = []) {
  return t
    .map((e) => `${e.name || n("Insumo")} ${M(e.amount, e.unit)}`)
    .join("; ");
}
export function timerRunningSummaryParts(t = {}) {
  return t.context === "mash" || t.phase === "mash"
    ? { action: l(t.label) || n("Mostura"), target: t.detail || "-", info: "" }
    : t.eventType === "boil-end"
      ? { action: n("Fim da"), target: n("fervura") }
      : t.eventType === "hopstand-end"
        ? { action: n("Fim do"), target: n("hopstand") }
        : {
...
export function timerDoneSummaryParts(t = {}) {
  return t.eventType === "mash-next"
    ...
    : t.eventType === "mash-end"
      ? { action: n("Finalizar"), target: n("mostura"), info: n("agora") }
      : t.eventType === "boil-end"
        ? /hopstand/i.test(t.eventActionLabel || "")
          ? { action: n("Iniciar"), target: n("hopstand"), info: n("agora") }
          : { action: n("Finalizar"), target: n("fervura"), info: n("agora") }
        : t.eventType === "hopstand-end"
          ? { action: n("Finalizar"), target: n("hopstand"), info: n("agora") }
          : {
