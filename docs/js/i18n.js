...
export const LANGUAGES = ["pt", "en", "es"],
    LANGUAGE_STORAGE_KEY = "beerSchool.receitasDinamicas.fable.language.v1";
const m = {
    pt: I,
    en: V,
    es: F
...
export function initLanguage() {
    ...
    return r = LANGUAGES.includes(e) ? e : detectLanguage(globalThis.navigator ? .language), r
}
export function t(e, o) {
    const n = m[r] ? .[e] ? ? m.pt ? .[e] ? ? e;
    return o ? String(n).replace(/\{(\w+)\}/g, (s, i) => i in o ? String(o[i]) : s) : n
}
const L = [
    [/^Confirmar adição de hopstand (.+)$/, "Confirmar adi\xE7\xE3o de hopstand {x}"],
...
export function tEngine(e) {
    ...
    return n === null && (n = o), r === "en" ? n.replace(/(\d),(\d)/g, "$1.$2") : n
}
const c = () => r === "en",
    E = {
        pt: "pt-BR",
...
export function formatYeastAmount(e, o) {
    const n = a($(e, o));
    return r === "pt" || n === "-" ? n : n.replace(/^([\d.,]+)\s+(.+)$/, (s, i, u) => `${i} ${t(u)}`)
}
initLanguage();