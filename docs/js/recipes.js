...
import {
    findStyle as bt,
    FERMENTATION_PRESETS as Lt
} from "./library.js";
import {
    t as N
} from "./i18n.js";
export const FERMENTABLE_TYPES = ["Gr\xE3o", "A\xE7\xFAcar", "A\xE7\xFAcar n\xE3o ferment\xE1vel", "Extrato"];
const At = 122,
    St = {
        pacote: 148,
        sach\ u00EA: 148,
...
export const MY_RECIPES_KEY = "beerSchool.receitasDinamicas.fable.myRecipes.v1",
    BREW_HISTORY_KEY = "beerSchool.receitasDinamicas.fable.brewHistory.v1";
const Tt = 24;
...
export function sanitizeFermentationPressure(t = {}) {
    return t.pressurized ? {
        pressurized: !0,
        pressureAtm: Math.min(MAX_FERMENTATION_PRESSURE_ATM, Math.max(0, e(t.pressureAtm, 0)))
    } : {}
}
export function ebcToLovibond(t) {
    return Math.max(0, (e(t) / 1.97 + .76) / 1.3546)
}
export function lovibondToEbc(t) {
    return Math.max(0, (e(t) * 1.3546 - .76) * 1.97)
}
...
export function draftFromRecipe(t, a = {}) {
    ...
    return {
        ...
        name: a.copy ? N("{name} (c\xF3pia)", {
            name: r.name || N("Receita")
        }) : r.name || "",
        styleName: r.styleName || "",
        brewer: r.brewer || "",
        batchVolumeL: e(r.batchVolumeL, 20),
        boilTimeMin: e(r.boilTimeMin, 60),
        mashEfficiencyPct: e(r.mashEfficiencyPct, e(r.efficiencyPct, f.mashEfficiencyPct)),
        manualFg: r.fgManual ? e(r.fg) : "",
        equipmentProfileName: String(r.equipmentProfileName || ""),
        trubLossPct: Number.isFinite(Number(r.trubLossPct)) ? Math.min(.5, Math.max(0, e(r.trubLossPct))) : e(r.batchVolumeL) > 0 && Number.isFinite(Number(r.trubLossL)) ? Math.min(.5, Math.max(0, u(e(r.trubLossL) / e(r.batchVolumeL), 4))) : f.trubLossPct,
        evaporationPct: Math.min(40, Math.max(0, e(r.evaporationPct, f.evaporationPct))),
        whirlpoolNoChillMin: Math.min(120, Math.max(0, e(r.whirlpoolNoChillMin, WHIRLPOOL_NOCHILL_MIN))),
        whirlpoolTemperatureC: Math.min(100, Math.max(40, e(r.whirlpoolTemperatureC, HOPSTAND_DEFAULT_TEMP_C))),
        heatingRateCMin: Math.min(10, Math.max(0, e(r.heatingRateCMin, v))),
        fermentables: (r.fermentables || []).map(o => ({
            name: o.name || "",
            type: o.notFermentable && (o.type || "Gr\xE3o") === "A\xE7\xFAcar" ? "A\xE7\xFAcar n\xE3o ferment\xE1vel" : o.type || "Gr\xE3o",
        ...
        hops: (r.hops || []).map(o => ({
            ...
            use: o.use || "Fervura",
            timeMin: e(o.timeMin),
            temperatureC: Number.isFinite(Number(o.temperatureC)) ? e(o.temperatureC) : 90
        })),
        ...
        salts: ["CaCl2", "CaSO4", "MgSO4"].map(o => ({
            formula: o,
            amountG: (r.salts || []).filter(m => m.formula === o).reduce((m, s) => m + e(s.amountG), 0)
        })),
        miscs: (r.miscs || []).map(o => ({
            name: o.name || "",
            amount: e(o.amount),
...
function D(t) {
    return (t.fermentables || []).map((a, r) => {
        ...
        return {
            ...
            draftType: o,
            use: m ? "Mostura" : a.when === "Fermenta\xE7\xE3o" ? "Fermenta\xE7\xE3o" : "Fervura",
...
function tt(t, a, r) {
    return t === "A\xE7\xFAcar" ? At : t === "A\xE7\xFAcar n\xE3o ferment\xE1vel" ? 0 : t === "Extrato" ? a : Math.min(100, Math.max(30, a + r))
}
...
export function tinsethUtilization(t, a) {
    return 1.65 * Math.pow(125e-6, t - 1) * (1 - Math.exp(-.04 * Math.max(0, e(a)))) / 4.15
}
const U = [
    [64, .06],
    [65, .07],
...
export function whirlpoolOptions(t = {}) {
    return {
        noChillMin: Math.min(120, Math.max(0, e(t.whirlpoolNoChillMin, WHIRLPOOL_NOCHILL_MIN))),
        temperatureC: Math.min(100, Math.max(40, e(t.whirlpoolTemperatureC, HOPSTAND_DEFAULT_TEMP_C)))
    }
}
export function editorIbuContributions(t, a, r, o = {}) {
    const m = Math.max(.1, e(r, 20)) * IBU_VOLUME_FACTOR,
        s = whirlpoolOptions(o),
        n = p => tinsethUtilization(a, Math.max(0, p)),
        M = (t || []).filter(p => ["Hopstand", "Whirlpool"].includes(p.use) && e(p.timeMin) > 0).reduce((p, g) => e(g.timeMin) > e(p ? .timeMin) ? g : p, null),
    ...
    return (t || []).map(p => {
        ...
        if (y > 0 && (P || g === "Fervura" || g === "First wort")) {
            const R = P ? e(p.temperatureC) > 0 ? e(p.temperatureC) : s.temperatureC : E,
...
export function computeTargets(t) {
    const a = Math.max(.1, e(t.batchVolumeL, 20)),
        r = Math.max(1, e(t.mashEfficiencyPct, 65)),
        o = Math.max(1, e(t.boilTimeMin, 60)),
        m = Math.min(.5, Math.max(0, e(t.trubLossPct, f.trubLossPct))),
    ...
    const L = p ? P / p : tt("Gr\xE3o", g, A),
        ...
        z = pt(i, a + n),
        J = u(Pt(E, O), 1),
        G = (t.yeasts || []).reduce((h, b) => h + e(b.amount) * (St[b.unit] ? ? 0), 0),
        j = ft(E),
        nt = G > 0 && j > .5 ? u(G / (a * j), 2) : 0,
        q = i.filter(h => h.use === "Mostura").reduce((h, b) => h + e(b.amountKg), 0),
        rt = C * o / 60,
...
export function scaleFermentablesToOg(t, a) {
    ...
    return t.fermentables.forEach(n => {
...
function W(t, a, r) {
    const o = t.fermentables || [],
        m = Math.max(.1, e(t.batchVolumeL, 20)),
        s = Math.min(.5, Math.max(0, e(t.trubLossPct, f.trubLossPct))),
        n = {
            targetVolumeL: m + u(m * s, 2),
            mashEfficiencyPct: Math.max(1, e(t.mashEfficiencyPct, 65))
        },
        i = D(t),
        M = Number.isFinite(Number(r)) ? Math.max(1, T(e(r))) : i.reduce((P, L) => P + Math.max(0, T(_([L], n).og)), 0),
        l = a.reduce((P, L) => P + Math.max(0, e(L)), 0);
    if (!M || !l) return !1;
    const E = a.map(P => Math.max(0, e(P)) / l),
        p = i.map(P => Math.max(0, T(_([{ ...P,
            amountKg: 1
        }], n).og))),
...
function V(t) {
    return ["Fervura", "First wort"].includes(t.use || "Fervura") && e(t.timeMin) >= BITTERING_MIN_BOIL_MIN
}
export function scaleHopsToIbu(t, a) {
    const r = computeTargets(t),
        o = Math.max(.1, e(t.batchVolumeL, 20)),
        m = t.hops || [],
...
export function rescaleDraftForEquipment(t, a, r) {
    const o = Math.max(.1, e(t.batchVolumeL, 20)),
        m = Math.max(.1, e(r, o)),
        s = o / m;
    return (t.fermentables || []).forEach(n => {
        ...
        n.amount = u(e(n.amount) * s, 2)
    }), e(a ? .og) > 1 && (t.fermentables || []).some(n => e(n.amountKg) > 0) && scaleFermentablesToOg(t, e(a.og)), e(a ? .ibu) > 0 && scaleHopsToIbu(t, e(a.ibu)), {
        factor: u(s, 4)
    }
}
...
export function applyHopAlphaToAll(t, a, r) {
    ...
    let m = 0;
    return (t.hops || []).forEach(s => {
        String(s.name || "").trim().toLowerCase() === o && (s.alphaAcidPct = Math.max(0, e(r)), m += 1)
    }), m
}
export const PROFILES_KEY = "beerSchool.receitasDinamicas.fable.productionProfiles.v1",
    PRINCIPAL_PROFILE_KEY = "beerSchool.receitasDinamicas.fable.principalProfileId.v1";
...
export function saveProductionProfileEntry(t) {
    const a = customizedBaseWater(t.params ? .baseWaterProfile),
        r = {
            id: t.id || `profile-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,6)}`,
            ...
            params: {
                ...
                mashEfficiencyPct: Math.min(110, Math.max(20, e(t.params ? .mashEfficiencyPct, f.mashEfficiencyPct))),
                evaporationPct: Math.min(40, Math.max(0, e(t.params ? .evaporationPct, f.evaporationPct))),
                trubLossPct: Math.min(.5, Math.max(0, e(t.params ? .trubLossPct, f.trubLossPct))),
                grainAbsorptionLkg: Math.min(3, Math.max(0, e(t.params ? .grainAbsorptionLkg, f.grainAbsorptionLkg))),
                waterToGrainRatioLkg: Math.min(8, Math.max(1, e(t.params ? .waterToGrainRatioLkg, f.waterToGrainRatioLkg))),
                mashTunDeadSpaceL: Math.min(50, Math.max(0, e(t.params ? .mashTunDeadSpaceL, 0))),
                whirlpoolNoChillMin: Math.min(120, Math.max(0, e(t.params ? .whirlpoolNoChillMin, WHIRLPOOL_NOCHILL_MIN))),
                whirlpoolTemperatureC: Math.min(100, Math.max(40, e(t.params ? .whirlpoolTemperatureC, HOPSTAND_DEFAULT_TEMP_C))),
                heatingRateCMin: Math.min(10, Math.max(0, e(t.params ? .heatingRateCMin, v))),
                ...a ? {
                    baseWaterProfile: a
                } : {}
            }
...
export function productionParamsFromImportedRecipe(t, a = {}) {
    const r = Math.max(1, e(t.batchVolumeL, f.targetVolumeL)),
        o = (t.fermentables || []).filter(n => n.use === "Mostura").reduce((n, i) => n + e(i.amountKg), 0),
    ...
    return {
        ...
        waterToGrainRatioLkg: o > 0 && m > 0 ? Math.min(8, Math.max(1, u(m / o, 2))) : f.waterToGrainRatioLkg,
        ...Number.isFinite(s) ? {
...
export function deleteProductionProfileEntry(t) {
    ...
    try {
        localStorage.setItem(PROFILES_KEY, JSON.stringify(a)), getPrincipalProfileId() === t && localStorage.removeItem(PRINCIPAL_PROFILE_KEY)
    } catch {}
}
...
export const USER_LIBRARY_KEY = "beerSchool.receitasDinamicas.fable.userLibrary.v1";
...
export function medianBrewParameters(t = listBrewHistory(), a = 5) {
    ...
    const o = m => {
        const s = r.map(i => e(i[m])).filter(i => i > 0).sort((i, M) => i - M);
...
export function recipeFromDraft(t) {
    const a = computeTargets(t),
        r = Math.max(.1, e(t.batchVolumeL, 20)),
        o = Math.max(1, e(t.boilTimeMin, 60)),
    ...
    return {
        ...
        evaporationPct: Math.min(40, Math.max(0, e(t.evaporationPct, f.evaporationPct))),
        trubLossPct: Math.min(.5, Math.max(0, e(t.trubLossPct, f.trubLossPct))),
        whirlpoolNoChillMin: Math.min(120, Math.max(0, e(t.whirlpoolNoChillMin, WHIRLPOOL_NOCHILL_MIN))),
        whirlpoolTemperatureC: Math.min(100, Math.max(40, e(t.whirlpoolTemperatureC, HOPSTAND_DEFAULT_TEMP_C))),
        ...
        hops: (t.hops || []).map((n, i) => ({
            ...
            use: n.use || "Fervura",
            timeMin: e(n.timeMin),
            temperatureC: ["Hopstand", "Whirlpool"].includes(n.use) ? e(n.temperatureC, 90) : void 0,
        ...
        salts: (t.salts || []).filter(n => e(n.amountG) > 0).map((n, i) => ({
            id: `s${i}`,
            name: n.formula,
            formula: n.formula,
            amountG: u(e(n.amountG), 1),
        ...
        miscs: (t.miscs || []).filter(n => n.name).map((n, i) => ({
            id: `m${i}`,
            name: n.name,
            type: "Insumo",
            use: n.use || "Fervura",
            timeMin: n.use === "Mostura" || n.use === "Fermenta\xE7\xE3o" ? void 0 : e(n.timeMin),
            amount: e(n.amount),
            unit: n.unit || "g"
        })),
        ...
        mash: (t.mash || []).length ? t.mash.map((n, i) => ({
            id: `m${i}`,
            name: n.name || "Mostura",
            type: "Temperature",
...
export function saveBrewedRecipe(t) {
    ...
    return r && (a.id = r.id), saveMyRecipe(a, {
        isDraft: !1
    })
}
...
export function upsertBrewHistory(t, a) {
    if (!(!t || !t.recipe || !a)) try {
        const r = Et(t),
            o = r.props,
            m = r.analysis || {},
            s = !!(e(t.measurements ? .preBoil ? .volumeL) || e(t.measurements ? .postBoil ? .volumeL)),
            n = {
                id: a,
                ...
                grainAbsorptionLkg: u(w(m.grainAbsorptionLkg, o.grainAbsorptionLkg), 2),
                trubLossL: u(w(m.trubLossL, o.trubLossL), 2),
                waterToGrainRatioLkg: e(o.waterToGrainRatioLkg, 3),
                parameterCode: gt(r)
            },
            i = listBrewHistory().filter(M => M.id !== a);
        i.unshift(n), localStorage.setItem(BREW_HISTORY_KEY, JSON.stringify(i.slice(0, Tt)))
    } catch {}
}
...
export function recipeToBeerXml(t) {
    const a = recipeFromDraft(t),
        ...
        o = {
            Fervura: "Boil",
            Mostura: "Mash",
            Fermenta\ u00E7\ u00E3o: "Primary",
            Envase: "Bottling"
        },
        m = [],
        s = a.fermentation;
    return s[0] && m.push(c("PRIMARY_TEMP", s[0].temperatureC), c("PRIMARY_AGE", s[0].days)), s[1] && m.push(c("SECONDARY_TEMP", s[1].temperatureC), c("SECONDARY_AGE", s[1].days)), s[2] && m.push(c("TERTIARY_TEMP", s[2].temperatureC), c("TERTIARY_AGE", s[2].days)), s[3] && m.push(c("AGE_TEMP", s[3].temperatureC), c("AGE", s[3].days)), ['<?xml version="1.0" encoding="UTF-8"?>', "<RECIPES><RECIPE>", c("NAME", a.name), c("VERSION", 1), c("TYPE", "All Grain"), c("BREWER", a.brewer), c("BATCH_SIZE", a.batchVolumeL), c("BOIL_SIZE", a.boilSize), c("BOIL_TIME", a.boilTimeMin), c("EFFICIENCY", a.efficiencyPct), c("MASH_EFFICIENCY", a.mashEfficiencyPct), c("OG", a.og), c("FG", a.fg), c("IBU", a.ibu), c("EST_COLOR", `${a.colorEbc} EBC`), ...m, `<STYLE>${c("NAME",a.styleName)}<VERSION>1</VERSION></STYLE>`, `<EQUIPMENT>${c("NAME","Editor Fable")}${c("BOIL_SIZE",a.boilSize)}${c("BOIL_TIME",a.boilTimeMin)}${c("TRUB_CHILLER_LOSS",a.trubLossL)}${c("BOIL_OFF_PER_HOUR",a.evaporationLh)}</EQUIPMENT>`, "<FERMENTABLES>", ...a.fermentables.map(i => `<FERMENTABLE>${c("NAME",i.name)}${c("VERSION",1)}${c("TYPE",i.type==="A\xE7\xFAcar"?"Sugar":i.type==="Extrato"?"Dry Extract":"Grain")}${c("AMOUNT",i.amountKg)}${c("YIELD",i.yieldPct)}${c("COLOR",u(i.colorLovibond,2))}${i.use==="Fermenta\xE7\xE3o"?c("ADD_AFTER_BOIL","TRUE"):i.use==="Fervura"?c("ADD_AFTER_BOIL","FALSE")+c("TIME",e(i.timeMin,10)):""}</FERMENTABLE>`), "</FERMENTABLES>", "<HOPS>", ...a.hops.map(i => `<HOP>${c("NAME",i.name)}${c("VERSION",1)}${c("ALPHA",i.alphaAcidPct)}${c("AMOUNT",u(i.amountG/1e3,4))}${c("USE",r[i.use]||"Boil")}${c("TIME",i.timeMin)}${Number.isFinite(Number(i.temperatureC))?c("HOP_TEMP",i.temperatureC):""}${c("FORM","Pellet")}</HOP>`), "</HOPS>", "<YEASTS>", ...a.yeasts.map(i => `<YEAST>${c("NAME",i.name)}${c("VERSION",1)}${c("ATTENUATION",i.attenuationPct)}${c("AMOUNT",i.amount)}${c("DISPLAY_AMOUNT",`${i.amount} ${i.unit}`)}</YEAST>`), "</YEASTS>", "<MISCS>", ...a.salts.map(i => `<MISC>${c("NAME",i.formula)}${c("VERSION",1)}${c("TYPE","Water Agent")}${c("USE","Mash")}${c("AMOUNT",u(i.amountG/1e3,5))}${c("AMOUNT_IS_WEIGHT","TRUE")}${c("DISPLAY_AMOUNT",`${i.amountG} g`)}</MISC>`), ...a.miscs.map(i => `<MISC>${c("NAME",i.name)}${c("VERSION",1)}${c("TYPE","Other")}${c("USE",o[i.use]||"Boil")}${Number.isFinite(Number(i.timeMin))?c("TIME",i.timeMin):""}${c("AMOUNT",i.unit==="g"?e(i.amount)/1e3:e(i.amount))}${c("AMOUNT_IS_WEIGHT",i.unit==="g"||i.unit==="kg"?"TRUE":"FALSE")}${c("DISPLAY_AMOUNT",`${i.amount} ${i.unit}`)}</MISC>`), "</MISCS>", `<WATERS><WATER>${c("NAME","\xC1gua base")}${c("VERSION",1)}${c("CALCIUM",a.baseWaterProfile.calciumPpm)}${c("MAGNESIUM",a.baseWaterProfile.magnesiumPpm)}${c("SODIUM",a.baseWaterProfile.sodiumPpm)}${c("CHLORIDE",a.baseWaterProfile.chloridePpm)}${c("SULFATE",a.baseWaterProfile.sulfatePpm)}${c("BICARBONATE",a.baseWaterProfile.bicarbonatePpm)}</WATER></WATERS>`, "<MASH><MASH_STEPS>", ...a.mash.map(i => `<MASH_STEP>${c("NAME",i.name)}${c("VERSION",1)}${c("TYPE","Temperature")}${c("STEP_TEMP",i.temperatureC)}${c("STEP_TIME",i.timeMin)}</MASH_STEP>`), "</MASH_STEPS></MASH>", "</RECIPE></RECIPES>"].join(`
`)
}
