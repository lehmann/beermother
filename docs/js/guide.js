...
function O(e, t, a) {
    ...
    return !r || r.status === "pending" ? "pending" : r.action === "Sem corre\xE7\xE3o" || N(r) ? "none" : e ? .correctionAccepted ? .[a] ? "done" : (e ? .correctionRounds ? .[a] || []).length > 0 ? "verifying" : "actionable"
}

function V(e, t, a) {
    const r = e ? .timerEvents || [],
        c = t === "mash" ? "mash-end" : t === "boil" ? "boil-end" : "";
    return c && r.some(n => String(n.key || "").startsWith(c)) ? !0 : r.some(n => String(n.key || "").includes(String(a)))
}
export function brewGuideSteps(e, t = {}, a = "guia", r = {}) {
    ...
    n({
        ...
        phase: "mash",
        type: "check",
        title: i("\xC1gua da mostura \xB7 {vol}", {
            vol: b(l.mashWater, 1)
        }),
    ...
    }), (e.mashAdditions || []).forEach(o => n({
        id: `mash-add-${h(o)}`,
        phase: "mash",
        type: "check",
        title: i("Adicione {name} na mostura", {
            name: o.name
        }),
    ...
    let p = -1;
    d.forEach((o, s) => {
        S.some(g => String(g.key || "").includes(String(o.id))) && (p = Math.max(p, s))
    }), d.forEach((o, s) => {
        ...
        n({
            ...
            phase: "mash",
            type: "timer",
            title: g ? i("Aquecer para {step}", {
                step: o.eventTimeLabel || i("o pr\xF3ximo patamar")
            }) + (o.eventTargetLabel ? ` \xB7 ${o.eventTargetLabel}` : "") : `${o.label} \xB7 ${o.detail}`,
            detail: g ? i("~{min} min (estimado)", {
                min: y(o.durationMin, 0)
    ...
    n({
    ...
    }), ["actionable", "verifying", "done"].includes(M) && n({
        ...
        ref: {
            tab: "mash",
            anchor: "correction-pre"
        },
        done: M === "done"
    });
    const W = K(e),
        x = W.flatMap(o => (o.additions || []).filter(s => s.use === "First wort"));
    x.length && n({
        id: "boil-firstwort",
        phase: "boil",
        type: "check",
        title: i("Adicione {names}", {
            names: x.map(o => o.name).join(", ")
        }),
        ...
        detail: i("{min} min de fervura", {
            min: y(e.recipe.boilTimeMin, 0)
        }),
        ref: {
            tab: "boil",
            anchor: "boil-hops"
    ...
    n({
    ...
    }), ["actionable", "verifying", "done"].includes($) && n({
        ...
        n({
            ...
            detail: i("hopstand {time}", {
                time: o.eventTimeLabel
            }) + (g ? i(" a {t}\xB0C", {
                t: y(g, 0)
            }) : ""),
            ref: {
    ...
    return A.forEach((o, s) => {
        o.status = o.done ? "done" : s === L ? "current" : s < q ? "skipped" : "pending"
    }), A
}

function Y(e) {
    return e.type === "leitura" || e.type === "correcao" || Array.isArray(e.checks) && e.checks.length ? "essencial" : "guia"
}
const P = [{
        at: "start",
        id: "op-review",
        ...
        phase: "prepare",
        icon: "scale",
        title: e => i("Moa {mass} de malte (se em gr\xE3o)", {
        ...
        title: e => {
            ...
            return t ? i("Aque\xE7a {vol} a ~{t} \xB0C para a mostura", {
        ...
        phase: "mash",
        icon: "water",
        title: e => i("Aque\xE7a {vol} a 76\u201378 \xB0C para a lavagem", {
            vol: b(e.spargeEffectiveL ? ? e.volumes.sparge, 1)
        }),
        tip: e => i("Aproveite para aquecer a \xE1gua de lavagem enquanto faz a mostura \u2014 {vol} a 76\u201378 \xB0C", {
            vol: b(e.spargeEffectiveL ? ? e.volumes.sparge, 1)
        })
    }, {
        after: "mash-sparge",
        ...
        phase: "mash",
        icon: "boil",
        title: e => i("Transfira o mosto para a panela \u2014 alvo {vol}", {
        ...
        title: e => {
            ...
            return t ? i("Adicione a levedura \u2014 {yeast}", {
                yeast: t
        ...
        title: e => {
            ...
            return t ? i("Leve \xE0 geladeira \u2014 fermenta\xE7\xE3o a {t} \xB0C", {
                t: y(t, 0)
...
export function copilotoCompanions(e, t, a = "copiloto") {
    return a !== "copiloto" ? [] : P.filter(r => r.kind === "companion" && r.phase === t).map(r => ({
        id: r.id,
        phase: r.phase,
        icon: r.icon || "check",
        title: C(r.title, e),
...
function X(e, t, a) {
    const r = l => ({
        ...
        n = [...c("at", "start")];
    return e.forEach(l => {
        c("before", l.id).forEach(u => n.push(u)), n.push(l), c("after", l.id).forEach(u => n.push(u))
    }), n
}

function _(e, t, a) {
    return t ? .correctionAccepted ? .[a] ? i("desvio aceito \u2014 seguindo como est\xE1") : (t ? .correctionRounds ? .[a] || []).length > 0 ? i("aplicada \u2014 confira ou conclua") : !e || e.status === "pending" ? i("aguardando a leitura") : e.action === "Sem corre\xE7\xE3o" ? i("sem corre\xE7\xE3o necess\xE1ria") : `${i(e.action)} \xB7 ${b(e.targetVolumeL,2)}`
}

function z(e) {
    return e === "none" ? i("no alvo \u2014 sem corre\xE7\xE3o \u2713") : e === "actionable" || e === "verifying" || e === "done" ? i("fora do alvo \u2014 veja o ajuste") : ""
}

function J(e, t) {
    const a = new Set(t.map(c => c.id)),
        r = (e.boilAdditions || []).find(c => a.has(c.id) && Number.isFinite(Number(c.temperatureC)));
...
export function mashStepStates(e, t = {}) {
    ...
    let d = l ? 0 : -1;
    c.forEach((p, v) => {
        n.some(m => String(m.key || "").includes(String(p.id))) && (d = Math.max(d, v))
    });
    let S = -1;
    const I = new Map;
    return c.forEach((p, v) => {
        ...
        if (u) return "done";
        const m = I.get(v);
        return m === void 0 || d < 0 ? "pending" : m < d ? "done" : m === d || m === d + 1 && c[d] ? .label === "Aquecimento" ? "current" : "pending"
    })
}
