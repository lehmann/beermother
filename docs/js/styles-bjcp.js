export const BJCP_STYLES = [{
    code: "1A",
    name: "American Light Lager",
    og: [1.028, 1.04],
    fg: [.998, 1.008],
    abv: [2.8, 4.2],
    ibu: [8, 12],
    ebc: [4, 6]
}, {
    code: "1B",
    name: "American Lager",
    og: [1.04, 1.05],
    fg: [1.004, 1.01],
    abv: [4.2, 5.3],
    ibu: [8, 18],
    ebc: [4, 7]
}, {
    code: "1C",
    name: "Cream Ale",
    og: [1.042, 1.055],
    fg: [1.006, 1.012],
    abv: [4.2, 5.6],
    ibu: [8, 20],
    ebc: [4, 10]
}, {
    code: "1D",
    name: "American Wheat Beer",
    og: [1.04, 1.055],
    fg: [1.008, 1.013],
    abv: [4, 5.5],
    ibu: [15, 30],
    ebc: [6, 12]
}, {
    code: "2A",
    name: "International Pale Lager",
    og: [1.042, 1.05],
    fg: [1.008, 1.012],
    abv: [4.5, 6],
    ibu: [18, 25],
    ebc: [4, 12]
}, {
    code: "2B",
    name: "International Amber Lager",
    og: [1.042, 1.055],
    fg: [1.008, 1.014],
    abv: [4.5, 6],
    ibu: [8, 25],
    ebc: [12, 28]
}, {
    code: "2C",
    name: "International Dark Lager",
    og: [1.044, 1.056],
    fg: [1.008, 1.012],
    abv: [4.2, 6],
    ibu: [8, 20],
    ebc: [28, 59]
}, {
    code: "3A",
    name: "Czech Pale Lager",
    og: [1.028, 1.044],
    fg: [1.008, 1.014],
    abv: [3, 4.1],
    ibu: [20, 35],
    ebc: [6, 12]
}, {
    code: "3B",
    name: "Czech Premium Pale Lager",
    og: [1.044, 1.06],
    fg: [1.013, 1.017],
    abv: [4.2, 5.8],
    ibu: [30, 45],
    ebc: [7, 12]
}, {
    code: "3C",
    name: "Czech Amber Lager",
    og: [1.044, 1.06],
    fg: [1.013, 1.017],
    abv: [4.4, 5.8],
    ibu: [20, 35],
    ebc: [20, 32]
}, {
    code: "3D",
    name: "Czech Dark Lager",
    og: [1.044, 1.06],
    fg: [1.013, 1.017],
    abv: [4.4, 5.8],
    ibu: [18, 34],
    ebc: [34, 69]
}, {
    code: "4A",
    name: "Munich Helles",
    og: [1.044, 1.048],
    fg: [1.006, 1.012],
    abv: [4.7, 5.4],
    ibu: [16, 22],
    ebc: [6, 10]
}, {
    code: "4B",
    name: "Festbier",
    og: [1.054, 1.057],
    fg: [1.01, 1.012],
    abv: [5.8, 6.3],
    ibu: [18, 25],
    ebc: [8, 12]
}, {
    code: "4C",
    name: "Helles Bock",
    og: [1.064, 1.072],
    fg: [1.011, 1.018],
    abv: [6.3, 7.4],
    ibu: [23, 35],
    ebc: [12, 18]
}, {
    code: "5A",
    name: "German Leichtbier",
    og: [1.026, 1.034],
    fg: [1.006, 1.01],
    abv: [2.4, 3.6],
    ibu: [15, 28],
    ebc: [3, 8]
}, {
    code: "5B",
    name: "K\xF6lsch",
    og: [1.044, 1.05],
    fg: [1.007, 1.011],
    abv: [4.4, 5.2],
    ibu: [18, 30],
    ebc: [7, 10]
}, {
    code: "5C",
    name: "German Helles Exportbier",
    og: [1.05, 1.058],
    fg: [1.008, 1.015],
    abv: [5, 6],
    ibu: [20, 30],
    ebc: [8, 12]
}, {
    code: "5D",
    name: "German Pils",
    og: [1.044, 1.05],
    fg: [1.008, 1.013],
    abv: [4.4, 5.2],
    ibu: [22, 40],
    ebc: [4, 8]
}, {
    code: "6A",
    name: "M\xE4rzen",
    og: [1.054, 1.06],
    fg: [1.01, 1.014],
    abv: [5.6, 6.3],
    ibu: [18, 24],
    ebc: [16, 34]
}, {
    code: "6B",
    name: "Rauchbier",
    og: [1.05, 1.057],
    fg: [1.012, 1.016],
    abv: [4.8, 6],
    ibu: [20, 30],
    ebc: [24, 43]
}, {
    code: "6C",
    name: "Dunkles Bock",
    og: [1.064, 1.072],
    fg: [1.013, 1.019],
    abv: [6.3, 7.2],
    ibu: [20, 27],
    ebc: [28, 43]
}, {
    code: "7A",
    name: "Vienna Lager",
    og: [1.048, 1.055],
    fg: [1.01, 1.014],
    abv: [4.7, 5.5],
    ibu: [18, 30],
    ebc: [18, 30]
}, {
    code: "7B",
    name: "Altbier",
    og: [1.044, 1.052],
    fg: [1.008, 1.014],
    abv: [4.3, 5.5],
    ibu: [25, 50],
    ebc: [18, 34]
}, {
    code: "8A",
    name: "Munich Dunkel",
    og: [1.048, 1.056],
    fg: [1.01, 1.016],
    abv: [4.5, 5.6],
    ibu: [18, 28],
    ebc: [34, 55]
}, {
    code: "8B",
    name: "Schwarzbier",
    og: [1.046, 1.052],
    fg: [1.01, 1.016],
    abv: [4.4, 5.4],
    ibu: [20, 35],
    ebc: [37, 59]
}, {
    code: "9A",
    name: "Doppelbock",
    og: [1.072, 1.112],
    fg: [1.016, 1.024],
    abv: [7, 10],
    ibu: [16, 26],
    ebc: [12, 49]
}, {
    code: "9B",
    name: "Eisbock",
    og: [1.078, 1.12],
    fg: [1.02, 1.035],
    abv: [9, 14],
    ibu: [25, 35],
    ebc: [34, 59]
}, {
    code: "9C",
    name: "Baltic Porter",
    og: [1.06, 1.09],
    fg: [1.016, 1.024],
    abv: [6.5, 9.5],
    ibu: [20, 40],
    ebc: [34, 59]
}, {
    code: "10A",
    name: "Weissbier",
    og: [1.044, 1.053],
    fg: [1.008, 1.014],
    abv: [4.3, 5.6],
    ibu: [8, 15],
    ebc: [4, 12]
}, {
    code: "10B",
    name: "Dunkles Weissbier",
    og: [1.044, 1.057],
    fg: [1.008, 1.014],
    abv: [4.3, 5.6],
    ibu: [10, 18],
    ebc: [28, 45]
}, {
    code: "10C",
    name: "Weizenbock",
    og: [1.064, 1.09],
    fg: [1.015, 1.022],
    abv: [6.5, 9],
    ibu: [15, 30],
    ebc: [12, 49]
}, {
    code: "11A",
    name: "Ordinary Bitter",
    og: [1.03, 1.039],
    fg: [1.007, 1.011],
    abv: [3.2, 3.8],
    ibu: [25, 35],
    ebc: [16, 28]
}, {
    code: "11B",
    name: "Best Bitter",
    og: [1.04, 1.048],
    fg: [1.008, 1.012],
    abv: [3.8, 4.6],
    ibu: [25, 40],
    ebc: [16, 32]
}, {
    code: "11C",
    name: "Strong Bitter",
    og: [1.048, 1.06],
    fg: [1.01, 1.016],
    abv: [4.6, 6.2],
    ibu: [30, 50],
    ebc: [16, 36]
}, {
    code: "12A",
    name: "British Golden Ale",
    og: [1.038, 1.053],
    fg: [1.006, 1.012],
    abv: [3.8, 5],
    ibu: [20, 45],
    ebc: [4, 10]
}, {
    code: "12B",
    name: "Australian Sparkling Ale",
    og: [1.038, 1.05],
    fg: [1.004, 1.006],
    abv: [4.5, 6],
    ibu: [20, 35],
    ebc: [8, 14]
}, {
    code: "12C",
    name: "English IPA",
    og: [1.05, 1.07],
    fg: [1.01, 1.015],
    abv: [5, 7.5],
    ibu: [40, 60],
    ebc: [12, 28]
}, {
    code: "13A",
    name: "Dark Mild",
    og: [1.03, 1.038],
    fg: [1.008, 1.013],
    abv: [3, 3.8],
    ibu: [10, 25],
    ebc: [28, 49]
}, {
    code: "13B",
    name: "British Brown Ale",
    og: [1.04, 1.052],
    fg: [1.008, 1.013],
    abv: [4.2, 5.9],
    ibu: [20, 30],
    ebc: [24, 43]
}, {
    code: "13C",
    name: "English Porter",
    og: [1.04, 1.052],
    fg: [1.008, 1.014],
    abv: [4, 5.4],
    ibu: [18, 35],
    ebc: [39, 59]
}, {
    code: "14A",
    name: "Scottish Light",
    og: [1.03, 1.035],
    fg: [1.01, 1.013],
    abv: [2.5, 3.3],
    ibu: [10, 20],
    ebc: [34, 49]
}, {
    code: "14B",
    name: "Scottish Heavy",
    og: [1.035, 1.04],
    fg: [1.01, 1.015],
    abv: [3.3, 3.9],
    ibu: [10, 20],
    ebc: [24, 39]
}, {
    code: "14C",
    name: "Scottish Export",
    og: [1.04, 1.06],
    fg: [1.01, 1.016],
    abv: [3.9, 6],
    ibu: [15, 30],
    ebc: [24, 39]
}, {
    code: "15A",
    name: "Irish Red Ale",
    og: [1.036, 1.046],
    fg: [1.01, 1.014],
    abv: [3.8, 5],
    ibu: [18, 28],
    ebc: [18, 28]
}, {
    code: "15B",
    name: "Irish Stout",
    og: [1.036, 1.044],
    fg: [1.007, 1.011],
    abv: [4, 4.5],
    ibu: [25, 45],
    ebc: [49, 79]
}, {
    code: "15C",
    name: "Irish Extra Stout",
    og: [1.052, 1.062],
    fg: [1.01, 1.014],
    abv: [5, 6.5],
    ibu: [35, 50],
    ebc: [59, 79]
}, {
    code: "16A",
    name: "Sweet Stout",
    og: [1.044, 1.06],
    fg: [1.012, 1.024],
    abv: [4, 6],
    ibu: [20, 40],
    ebc: [59, 79]
}, {
    code: "16B",
    name: "Oatmeal Stout",
    og: [1.045, 1.065],
    fg: [1.01, 1.018],
    abv: [4.2, 5.9],
    ibu: [25, 40],
    ebc: [43, 79]
}, {
    code: "16C",
    name: "Tropical Stout",
    og: [1.056, 1.075],
    fg: [1.01, 1.018],
    abv: [5.5, 8],
    ibu: [30, 50],
    ebc: [59, 79]
}, {
    code: "16D",
    name: "Foreign Extra Stout",
    og: [1.056, 1.075],
    fg: [1.01, 1.018],
    abv: [6.3, 8],
    ibu: [50, 70],
    ebc: [59, 79]
}, {
    code: "17A",
    name: "British Strong Ale",
    og: [1.055, 1.08],
    fg: [1.015, 1.022],
    abv: [5.5, 8],
    ibu: [30, 60],
    ebc: [16, 43]
}, {
    code: "17B",
    name: "Old Ale",
    og: [1.055, 1.088],
    fg: [1.015, 1.022],
    abv: [5.5, 9],
    ibu: [30, 60],
    ebc: [20, 43]
}, {
    code: "17C",
    name: "Wee Heavy",
    og: [1.07, 1.13],
    fg: [1.018, 1.04],
    abv: [6.5, 10],
    ibu: [17, 35],
    ebc: [28, 49]
}, {
    code: "17D",
    name: "English Barley wine",
    og: [1.08, 1.12],
    fg: [1.018, 1.03],
    abv: [8, 12],
    ibu: [35, 70],
    ebc: [16, 43]
}, {
    code: "18A",
    name: "Blonde Ale",
    og: [1.038, 1.054],
    fg: [1.008, 1.013],
    abv: [3.8, 5.5],
    ibu: [15, 28],
    ebc: [6, 12]
}, {
    code: "18B",
    name: "American Pale Ale",
    og: [1.045, 1.06],
    fg: [1.01, 1.015],
    abv: [4.5, 6.2],
    ibu: [30, 50],
    ebc: [10, 20]
}, {
    code: "19A",
    name: "American Amber Ale",
    og: [1.045, 1.06],
    fg: [1.01, 1.015],
    abv: [4.5, 6.2],
    ibu: [25, 40],
    ebc: [20, 34]
}, {
    code: "19B",
    name: "California Common",
    og: [1.048, 1.054],
    fg: [1.011, 1.014],
    abv: [4.5, 5.5],
    ibu: [30, 45],
    ebc: [18, 28]
}, {
    code: "19C",
    name: "American Brown Ale",
    og: [1.045, 1.06],
    fg: [1.01, 1.016],
    abv: [4.3, 6.2],
    ibu: [20, 30],
    ebc: [36, 69]
}, {
    code: "20A",
    name: "American Porter",
    og: [1.05, 1.07],
    fg: [1.012, 1.018],
    abv: [4.8, 6.5],
    ibu: [25, 50],
    ebc: [43, 79]
}, {
    code: "20B",
    name: "American Stout",
    og: [1.05, 1.075],
    fg: [1.01, 1.022],
    abv: [5, 7],
    ibu: [35, 75],
    ebc: [59, 79]
}, {
    code: "20C",
    name: "Imperial Stout",
    og: [1.075, 1.115],
    fg: [1.018, 1.03],
    abv: [8, 12],
    ibu: [50, 90],
    ebc: [59, 79]
}, {
    code: "21A",
    name: "American IPA",
    og: [1.056, 1.07],
    fg: [1.008, 1.014],
    abv: [5.5, 7.5],
    ibu: [40, 70],
    ebc: [12, 28]
}, {
    code: "21B",
    name: "Specialty IPA: Belgian IPA",
    og: [1.058, 1.08],
    fg: [1.008, 1.016],
    abv: [6.2, 9.5],
    ibu: [50, 100],
    ebc: [10, 16]
}, {
    code: "21B",
    name: "Specialty IPA: Black IPA",
    og: [1.05, 1.085],
    fg: [1.01, 1.018],
    abv: [5.5, 9],
    ibu: [50, 90],
    ebc: [49, 79]
}, {
    code: "21B",
    name: "Specialty IPA: Brown IPA",
    og: [1.056, 1.07],
    fg: [1.008, 1.016],
    abv: [5.5, 7.5],
    ibu: [40, 70],
    ebc: [36, 69]
}, {
    code: "21B",
    name: "Specialty IPA: Brut IPA",
    og: [1.046, 1.057],
    fg: [.99, 1.004],
    abv: [6, 7.5],
    ibu: [20, 30],
    ebc: [4, 8]
}, {
    code: "21B",
    name: "Specialty IPA: Red IPA",
    og: [1.056, 1.07],
    fg: [1.008, 1.016],
    abv: [5.5, 7.5],
    ibu: [40, 70],
    ebc: [22, 34]
}, {
    code: "21B",
    name: "Specialty IPA: Rye IPA",
    og: [1.056, 1.075],
    fg: [1.008, 1.014],
    abv: [5.5, 8],
    ibu: [50, 75],
    ebc: [12, 28]
}, {
    code: "21B",
    name: "Specialty IPA: White IPA",
    og: [1.056, 1.065],
    fg: [1.01, 1.016],
    abv: [5.5, 7],
    ibu: [40, 70],
    ebc: [10, 12]
}, {
    code: "21B",
    name: "Specialty IPA",
    og: [0, 0],
    fg: [0, 0],
    abv: [0, 0],
    ibu: [0, 0],
    ebc: [0, 0]
}, {
    code: "21C",
    name: "Hazy IPA",
    og: [1.06, 1.085],
    fg: [1.01, 1.015],
    abv: [6, 9],
    ibu: [25, 60],
    ebc: [6, 14]
}, {
    code: "22A",
    name: "Double IPA",
    og: [1.065, 1.085],
    fg: [1.008, 1.018],
    abv: [7.5, 10],
    ibu: [60, 100],
    ebc: [12, 28]
}, {
    code: "22B",
    name: "American Strong Ale",
    og: [1.062, 1.09],
    fg: [1.014, 1.024],
    abv: [6.3, 10],
    ibu: [50, 100],
    ebc: [14, 35]
}, {
    code: "22C",
    name: "American Barleywine",
    og: [1.08, 1.12],
    fg: [1.016, 1.03],
    abv: [8, 12],
    ibu: [50, 100],
    ebc: [18, 36]
}, {
    code: "22D",
    name: "Wheatwine",
    og: [1.08, 1.12],
    fg: [1.016, 1.03],
    abv: [8, 12],
    ibu: [30, 60],
    ebc: [12, 28]
}, {
    code: "23A",
    name: "Berliner Weisse",
    og: [1.028, 1.032],
    fg: [1.003, 1.006],
    abv: [2.8, 3.8],
    ibu: [3, 8],
    ebc: [4, 6]
}, {
    code: "23B",
    name: "Flanders Red Ale",
    og: [1.048, 1.057],
    fg: [1.002, 1.012],
    abv: [4.6, 6.5],
    ibu: [10, 25],
    ebc: [20, 34]
}, {
    code: "23C",
    name: "Oud Bruin",
    og: [1.04, 1.074],
    fg: [1.008, 1.012],
    abv: [4, 8],
    ibu: [20, 25],
    ebc: [33, 43]
}, {
    code: "23D",
    name: "Lambic",
    og: [1.04, 1.054],
    fg: [1.001, 1.01],
    abv: [5, 6.5],
    ibu: [0, 10],
    ebc: [6, 12]
}, {
    code: "23E",
    name: "Gueuze",
    og: [1.04, 1.054],
    fg: [1, 1.006],
    abv: [5, 8],
    ibu: [0, 10],
    ebc: [10, 12]
}, {
    code: "23F",
    name: "Fruit Lambic",
    og: [1.04, 1.06],
    fg: [1, 1.01],
    abv: [5, 7],
    ibu: [0, 10],
    ebc: [6, 14]
}, {
    code: "23G",
    name: "Gose",
    og: [1.036, 1.056],
    fg: [1.006, 1.01],
    abv: [4.2, 4.8],
    ibu: [5, 12],
    ebc: [6, 8]
}, {
    code: "24A",
    name: "Witbier",
    og: [1.044, 1.052],
    fg: [1.008, 1.012],
    abv: [4.5, 5.5],
    ibu: [8, 20],
    ebc: [4, 8]
}, {
    code: "24B",
    name: "Belgian Pale Ale",
    og: [1.048, 1.054],
    fg: [1.01, 1.014],
    abv: [4.8, 5.5],
    ibu: [20, 30],
    ebc: [16, 28]
}, {
    code: "24C",
    name: "Bi\xE8re de Garde",
    og: [1.06, 1.08],
    fg: [1.008, 1.016],
    abv: [6, 8.5],
    ibu: [18, 28],
    ebc: [12, 37]
}, {
    code: "25A",
    name: "Belgian Blond Ale",
    og: [1.062, 1.075],
    fg: [1.008, 1.018],
    abv: [6, 7.5],
    ibu: [15, 30],
    ebc: [8, 12]
}, {
    code: "25B",
    name: "Saison",
    og: [1.048, 1.065],
    fg: [1.002, 1.008],
    abv: [3.5, 9.5],
    ibu: [20, 35],
    ebc: [10, 43]
}, {
    code: "25C",
    name: "Belgian Golden Strong Ale",
    og: [1.07, 1.095],
    fg: [1.005, 1.016],
    abv: [7.5, 10.5],
    ibu: [22, 35],
    ebc: [6, 12]
}, {
    code: "26A",
    name: "Belgian Single",
    og: [1.044, 1.054],
    fg: [1.004, 1.01],
    abv: [4.8, 6],
    ibu: [25, 45],
    ebc: [6, 10]
}, {
    code: "26B",
    name: "Belgian Dubbel",
    og: [1.062, 1.075],
    fg: [1.008, 1.018],
    abv: [6, 7.6],
    ibu: [15, 25],
    ebc: [20, 34]
}, {
    code: "26C",
    name: "Belgian Tripel",
    og: [1.075, 1.085],
    fg: [1.008, 1.014],
    abv: [7.5, 9.5],
    ibu: [20, 40],
    ebc: [9, 14]
}, {
    code: "26D",
    name: "Belgian Dark Strong Ale",
    og: [1.075, 1.11],
    fg: [1.01, 1.024],
    abv: [8, 12],
    ibu: [20, 35],
    ebc: [24, 43]
}, {
    code: "27A",
    name: "Historical Beer: Kellerbier",
    og: [0, 0],
    fg: [0, 0],
    abv: [0, 0],
    ibu: [0, 0],
    ebc: [0, 0]
}, {
    code: "27A",
    name: "Historical Beer: Kentucky Common",
    og: [1.044, 1.055],
    fg: [1.01, 1.018],
    abv: [4, 5.5],
    ibu: [15, 30],
    ebc: [22, 39]
}, {
    code: "27A",
    name: "Historical Beer: Lichtenhainer",
    og: [1.032, 1.04],
    fg: [1.004, 1.008],
    abv: [3.5, 4.7],
    ibu: [5, 12],
    ebc: [6, 12]
}, {
    code: "27A",
    name: "Historical Beer: London Brown Ale",
    og: [1.033, 1.038],
    fg: [1.012, 1.015],
    abv: [2.8, 3.6],
    ibu: [15, 20],
    ebc: [43, 69]
}, {
    code: "27A",
    name: "Historical Beer: Piwo Grodziskie",
    og: [1.028, 1.032],
    fg: [1.006, 1.012],
    abv: [2.5, 3.3],
    ibu: [20, 35],
    ebc: [6, 12]
}, {
    code: "27A",
    name: "Historical Beer: Pre-Prohibition Lager",
    og: [1.044, 1.06],
    fg: [1.01, 1.015],
    abv: [4.5, 6],
    ibu: [25, 40],
    ebc: [6, 12]
}, {
    code: "27A",
    name: "Historical Beer: Pre-Prohibition Porter",
    og: [1.046, 1.06],
    fg: [1.01, 1.016],
    abv: [4.5, 6],
    ibu: [20, 30],
    ebc: [39, 59]
}, {
    code: "27A",
    name: "Historical Beer: Roggenbier",
    og: [1.046, 1.056],
    fg: [1.01, 1.014],
    abv: [4.5, 6],
    ibu: [10, 20],
    ebc: [28, 37]
}, {
    code: "27A",
    name: "Historical Beer: Sahti",
    og: [1.076, 1.12],
    fg: [1.016, 1.038],
    abv: [7, 11],
    ibu: [0, 15],
    ebc: [8, 43]
}, {
    code: "28A",
    name: "Brett Beer",
    og: [0, 0],
    fg: [0, 0],
    abv: [0, 0],
    ibu: [0, 0],
    ebc: [0, 0]
}, {
    code: "28B",
    name: "Mixed-Fermentation Sour Beer",
    og: [0, 0],
    fg: [0, 0],
    abv: [0, 0],
    ibu: [0, 0],
    ebc: [0, 0]
}, {
    code: "28C",
    name: "Wild Specialty Beer",
    og: [0, 0],
    fg: [0, 0],
    abv: [0, 0],
    ibu: [0, 0],
    ebc: [0, 0]
}, {
    code: "28D",
    name: "Straight Sour Beer",
    og: [1.048, 1.065],
    fg: [1.006, 1.013],
    abv: [4.5, 7],
    ibu: [3, 8],
    ebc: [4, 6]
}, {
    code: "29A",
    name: "Fruit Beer",
    og: [0, 0],
    fg: [0, 0],
    abv: [0, 0],
    ibu: [0, 0],
    ebc: [0, 0]
}, {
    code: "29B",
    name: "Fruit and Spice Beer",
    og: [0, 0],
    fg: [0, 0],
    abv: [0, 0],
    ibu: [0, 0],
    ebc: [0, 0]
}, {
    code: "29C",
    name: "Specialty Fruit Beer",
    og: [0, 0],
    fg: [0, 0],
    abv: [0, 0],
    ibu: [0, 0],
    ebc: [0, 0]
}, {
    code: "29D",
    name: "Grape Ale",
    og: [1.059, 1.075],
    fg: [1.004, 1.013],
    abv: [6, 8.5],
    ibu: [10, 30],
    ebc: [8, 16]
}, {
    code: "30A",
    name: "Spice, Herb, or Vegetable Beer",
    og: [0, 0],
    fg: [0, 0],
    abv: [0, 0],
    ibu: [0, 0],
    ebc: [0, 0]
}, {
    code: "30B",
    name: "Autumn Seasonal Beer",
    og: [0, 0],
    fg: [0, 0],
    abv: [0, 0],
    ibu: [0, 0],
    ebc: [0, 0]
}, {
    code: "30C",
    name: "Winter Seasonal Beer",
    og: [0, 0],
    fg: [0, 0],
    abv: [0, 0],
    ibu: [0, 0],
    ebc: [0, 0]
}, {
    code: "30D",
    name: "Specialty Spice Beer",
    og: [0, 0],
    fg: [0, 0],
    abv: [0, 0],
    ibu: [0, 0],
    ebc: [0, 0]
}, {
    code: "31A",
    name: "Alternative Grain Beer",
    og: [0, 0],
    fg: [0, 0],
    abv: [0, 0],
    ibu: [0, 0],
    ebc: [0, 0]
}, {
    code: "31B",
    name: "Alternative Sugar Beer",
    og: [0, 0],
    fg: [0, 0],
    abv: [0, 0],
    ibu: [0, 0],
    ebc: [0, 0]
}, {
    code: "32A",
    name: "Classic Style Smoked Beer",
    og: [0, 0],
    fg: [0, 0],
    abv: [0, 0],
    ibu: [0, 0],
    ebc: [0, 0]
}, {
    code: "32B",
    name: "Specialty Smoked Beer",
    og: [0, 0],
    fg: [0, 0],
    abv: [0, 0],
    ibu: [0, 0],
    ebc: [0, 0]
}, {
    code: "33A",
    name: "Wood-Aged Beer",
    og: [0, 0],
    fg: [0, 0],
    abv: [0, 0],
    ibu: [0, 0],
    ebc: [0, 0]
}, {
    code: "33B",
    name: "Specialty Wood-Aged Beer",
    og: [0, 0],
    fg: [0, 0],
    abv: [0, 0],
    ibu: [0, 0],
    ebc: [0, 0]
}, {
    code: "34A",
    name: "Commercial Specialty Beer",
    og: [0, 0],
    fg: [0, 0],
    abv: [0, 0],
    ibu: [0, 0],
    ebc: [0, 0]
}, {
    code: "34B",
    name: "Mixed-Style Beer",
    og: [0, 0],
    fg: [0, 0],
    abv: [0, 0],
    ibu: [0, 0],
    ebc: [0, 0]
}, {
    code: "34C",
    name: "Experimental Beer",
    og: [0, 0],
    fg: [0, 0],
    abv: [0, 0],
    ibu: [0, 0],
    ebc: [0, 0]
}, {
    code: "X1",
    name: "Dorada Pampeana",
    og: [1.042, 1.054],
    fg: [1.009, 1.013],
    abv: [4.3, 5.5],
    ibu: [15, 22],
    ebc: [6, 10]
}, {
    code: "X2",
    name: "IPA Argenta",
    og: [1.055, 1.065],
    fg: [1.008, 1.015],
    abv: [5, 5.5],
    ibu: [35, 60],
    ebc: [12, 30]
}, {
    code: "X3",
    name: "Italian Grape Ale",
    og: [1.055, 1.1],
    fg: [1.008, 1.015],
    abv: [4.5, 12],
    ibu: [6, 30],
    ebc: [8, 49]
}, {
    code: "X4",
    name: "Catharina Sour",
    og: [1.039, 1.048],
    fg: [1.004, 1.012],
    abv: [4, 5.5],
    ibu: [2, 8],
    ebc: [4, 12]
}, {
    code: "X5",
    name: "New Zealand Pilsner",
    og: [1.044, 1.056],
    fg: [1.009, 1.014],
    abv: [4.5, 5.8],
    ibu: [25, 45],
    ebc: [4, 12]
}];
const c = e => String(e || "").normalize("NFD").replace(/[̀-ͯ]/g, "").toLowerCase().replace(/[^a-z0-9]/g, "");
export function findBjcpStyle(e) {
    const a = String(e || "").trim().toLowerCase();
    if (!a) return null;
    const o = BJCP_STYLES.find(b => b.name.toLowerCase() === a);
    if (o) return o;
    const g = c(e);
    return BJCP_STYLES.find(b => c(b.name) === g) || null
}