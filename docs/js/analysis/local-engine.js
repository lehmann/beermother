...
// Recalibrated from 49 GT pairs (OLS); mash_temp via Gaussian enzyme model
// (β-amylase peak 63°C σ=4, α-amylase peak 70°C σ=4, weighted by step time)
const CTRL_COEFFS = {
  maltAroma: [
    1.1977, -0.0059, 0.1351, 0.0249, 0.0668, -0.018, 0.3389, 1.7707,
    -22.9452, 0.8477, -0.0092, 0.8141, -1.9577,
  ],
  hopAroma: [
    2.313, -0.001, -0.0402, 0.0116, -0.0002, -0.0146, 0.4702, -0.3801,
    3.1417, -1.3542, -0.3521, -0.0769, 7.8614,
  ],
  fermentationAroma: [
    4.6989, -0.0219, 0.0132, 0.0091, 0.2453, -0.037, -1.3211, -1.825,
    -0.8346, 0.9169, -0.3821, 0.3734, -0.0748,
  ],
  color: [
    0.3859, -0.0034, 0.1448, 0.0175, -0.0058, -0.0101, 0.152, -0.1378,
    -3.8802, 0.1775, 0.2223, 0.0851, -0.4472,
  ],
  clarity: [
    0.8741, 0.0054, 0.0238, 0.0069, -0.0292, -0.0172, -0.0968, -0.5968,
    3.5761, 1.6311, 0.0674, -0.1361, -0.1337,
  ],
  foamFormation: [
    2.0407, -0.0085, -0.0135, 0.0145, 0.0088, 0.0154, 0.1145, -0.0202,
    -1.4375, 2.1848, 0.1988, 0.2541, 0.8379,
  ],
  retention: [
    2.1294, -0.0126, -0.0183, 0.0107, 0.0395, 0.0084, 0.078, -0.1076,
    3.2852, 2.7445, 0.151, 0.2, 0.7098,
  ],
  maltFlavor: [
    1.0267, -0.0062, 0.1524, 0.0292, 0.0671, -0.0198, 0.3601, 2.1334,
    -25.8559, 0.9663, -0.005, 0.9719, -2.3192,
  ],
  hopFlavor: [
    2.3613, -0.001, -0.0365, 0.0095, -0.0027, -0.0168, 0.43, -0.2643,
    3.035, -1.233, -0.2962, -0.0506, 8.1056,
  ],
  fermentationFlavor: [
    4.6989, -0.0219, 0.0132, 0.0091, 0.2453, -0.037, -1.3211, -1.825,
    -0.8346, 0.9169, -0.3821, 0.3734, -0.0748,
  ...
  bitterness: [
    2.6575, 0.022, -0.0356, 0.0848, -0.2992, -0.0282, 0.1989, 2.6884,
    16.7412, -1.5648, 0.5464, 0.8263, 1.5558,
  ],
  balance: [
    2.4955, -0.0138, 0.0611, -0.0488, 0.1549, -0.0025, 0.1512, -0.6586,
    -2.0589, 1.7801, 0.1392, -0.2082, -1.3178,
  ],
  finish: [
    -8.3285, 0.0831, 0.0286, 0.0318, -0.6969, 0.1159, 0.1594, -0.1678,
    -1.4899, 1.6112, 0.1052, 0.317, -0.1187,
  ],
  body: [
    -10.1478, 0.083, 0.0377, 0.0383, -0.6642, 0.1408, 0.1698, 1.191,
    5.8511, 2.6634, 0.152, 0.2955, -0.4365,
  ],
  warming: [
    -1.2828, 0.035, 0.0283, 0.0239, -0.0424, -0.0046, -0.1607, -0.7378,
    -9.9572, 0.4441, -0.0588, -0.3007, -0.541,
  ],
  creaminess: [
    -1.4324, 0.0529, 0.0067, 0.0216, -0.4048, 0.0177, 0.0576, -0.8115,
    7.7755, 2.5203, 0.0296, 0.047, -0.3059,
  ],
  astringency: [
    -0.2771, -0.003, -0.0009, -0.0051, 0.022, 0.0079, -0.0648, -0.0242,
    4.4781, -0.2461, -0.1855, -0.0952, 0.1279,
  ],
};
...
export function runLocalAnalysis({ draft, seed = 1, styleSlug }) {
  ...
  // ── Recipe fractions ─────────────────────────────────────────────────────
  const mashFerms = fermentables.filter(
    (f) => f.when !== "Fermentação" && f.use !== "Fermentação",
  );
  ...
  const features = [
    ...
    abv,
    (() => {
          // Gaussian enzyme model: β-amylase peaks at 63°C σ=4, α-amylase peaks at 70°C σ=4
          // Weighted by time at each saccharification step → effective temperature 60–75°C
          const sac = mashSteps.filter(
            (s) => s && s.temperatureC >= 60 && s.temperatureC <= 75,
          );
          if (!sac.length) {
            const fb = [...mashSteps]
              .reverse()
              .find((s) => s && s.temperatureC < 78);
            return fb ? fb.temperatureC : 67;
          }
          const betaAct = (T) => Math.exp(-0.5 * ((T - 63) / 4) ** 2);
          const alphaAct = (T) => Math.exp(-0.5 * ((T - 70) / 4) ** 2);
          let bTotal = 0,
            aTotal = 0;
          for (const s of sac) {
            const t = s.timeMin || 1;
            bTotal += t * betaAct(s.temperatureC);
            aTotal += t * alphaAct(s.temperatureC);
          }
          const total = bTotal + aTotal;
          return total > 1e-9 ? 60 + (aTotal / total) * 15 : 67;
        })(),
    isLager,
    crystalFrac,
    roastFrac,
