...
export function svgNumber(t) {
    return Number(t).toFixed(2).replace(/\.?0+$/, "")
}
export function polylinePath(t) {
    return t.length ? t.map((n, c) => `${c?"L":"M"} ${svgNumber(n.x)} ${svgNumber(n.y)}`).join(" ") : ""
}
export function smoothPath(t) {
    ...
    for (let c = 1; c < t.length; c += 1) {
        const e = t[c - 1],
            r = t[c],
            $ = (e.x + r.x) / 2;
        n.push(`C ${svgNumber($)} ${svgNumber(e.y)} ${svgNumber($)} ${svgNumber(r.y)} ${svgNumber(r.x)} ${svgNumber(r.y)}`)
    }
    return n.join(" ")
}
...
function f(t, n) {
    ...
    const c = t.map((r, $) => `${$?"L":"M"} ${svgNumber(r.x)} ${svgNumber(r.y)}`).join(" "),
        e = [...n].reverse().map(r => `L ${svgNumber(r.x)} ${svgNumber(r.y)}`).join(" ");
    return `${c} ${e} Z`
}

function F(t, n, c) {
    if (!t || t.length < 2) return "";
    const e = o => t.filter(h => Number.isFinite(Number(h[o]))).map(h => ({
            x: n(h.day),
            y: c(s(h[o]))
        })),
...
export function fermentationChartSvg(t) {
    const e = {
        ...
        o = a => w => e.top + (1 - (s(w) - a.min) / Math.max(1, a.max - a.min)) * $,
        h = o(t.tempRange),
        i = o(t.extractRange),
        g = t.tempPoints.map(a => ({
            x: x(a.day),
            y: h(a.value)
        })),
        l = t.realTempPoints.map(a => ({
            x: x(a.day),
            y: h(a.value)
        })),
        m = t.extractPoints.map(a => ({
            x: x(a.day),
            y: i(a.value)
        })),
    ...
    <text x="${svgNumber(x(a))}" y="284" text-anchor="middle">${R(a)}</text>`).join(""),
        b = y(t.tempRange.min, t.tempRange.max, 4).map(a => `
    <line x1="${e.left}" y1="${svgNumber(h(a))}" x2="${720-e.right}" y2="${svgNumber(h(a))}" />
    <text x="${e.left-10}" y="${svgNumber(h(a)+4)}" text-anchor="end">${d(a,1)}</text>`).join(""),
        P = y(t.extractRange.min, t.extractRange.max, 4).map(a => `
    <text x="${720-e.right+10}" y="${svgNumber(i(a)+4)}" text-anchor="start">${d(a,1)}</text>`).join(""),
        M = m.map(a => `<circle class="chart-extract-point" cx="${svgNumber(a.x)}" cy="${svgNumber(a.y)}" r="4" />`).join(""),
        v = l.map(a => `<circle class="chart-temp-real-point" cx="${svgNumber(a.x)}" cy="${svgNumber(a.y)}" r="4" />`).join("");
