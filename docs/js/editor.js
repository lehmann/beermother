...
// ── XML helpers for equipment and batch Drive serialization ───────────────────
...
function xmlNum(doc, tag, fallback = 0) {
  const v = parseFloat(xmlText(doc, tag));
  return Number.isFinite(v) ? v : fallback;
}

// Serialize a production profile to a BeerXML-compatible <EQUIPMENTS> document.
function equipmentProfileToXml(profile) {
...
// Parse a BeerXML <EQUIPMENTS> document back into an internal production profile.
function equipmentProfileFromXml(xmlContent) {
...
// Serialize a brew entry to a BeerXML-compatible <RECIPES> document.
//
// The recipe section is generated by the existing recipeToBeerXml serializer
// (imported as Pa) so all fermentables, hops, yeasts, mash steps, etc. are
// included. Measured values and batch metadata are added as standard BeerXML
// fields on the <RECIPE> element. The complete internal payload is preserved
// in a <BM_PAYLOAD> CDATA block so the full round-trip is lossless.
function brewEntryToXml(entry) {
  ...
  // Build the recipe BeerXML using the existing serializer, then inject batch
  // fields before the closing </RECIPE> tag.
  let recipeXml = "";
  try {
    const draft = draftFromRecipe(recipe);
    recipeXml = Pa(draft);
  } catch {
    // Fallback: minimal recipe envelope if serialization fails.
    recipeXml = [
      '<?xml version="1.0" encoding="UTF-8"?>',
      "<RECIPES><RECIPE>",
  ...
  // Measured gravity values extracted from the session's measurement groups.
  ...
  // Build the batch-specific fields to inject before </RECIPE>.
  const batchFields = [
  ...
  // Inject batch fields and close the RECIPE/RECIPES tags.
  return recipeXml
    .replace("</RECIPE></RECIPES>", `${batchFields}\n</RECIPE></RECIPES>`);
}

// Parse a brew entry XML back into an internal brew entry object.
// The BM_PAYLOAD CDATA block is the authoritative source for the full payload.
function brewEntryFromXml(xmlContent) {
  const doc = new DOMParser().parseFromString(xmlContent, "application/xml");
  if (doc.querySelector("parsererror")) return null;
  const recipe = doc.getElementsByTagName("RECIPE")[0];
  const batch = doc.getElementsByTagName("BEERMOTHER_BATCH")[0] || recipe;
  if (!recipe && !batch) return null;
  const root = batch || recipe;
  const id = xmlText(root, "BM_ID");
  if (!id) return null;
  let payload = null;
  try {
    payload = JSON.parse(root.getElementsByTagName("BM_PAYLOAD")[0]?.textContent || "null");
  } catch { return null; }
  if (!payload) return null;
  return {
    id,
    status: xmlText(root, "BM_STATUS") || "active",
    recipeName: payload.recipe?.name || xmlText(root, "NAME") || "",
    styleName: payload.recipe?.styleName || "",
    updatedAt: xmlText(root, "BM_UPDATED_AT") || "",
    concludedAt: xmlText(root, "BM_CONCLUDED_AT") || "",
    payload,
  };
}
