...
export const PH_STAGES = [
  ...
  PH_ACIDS = [
    { id: "latico-85", label: "\xC1cido l\xE1tico 85%" },
    { id: "fosforico-10", label: "\xC1cido fosf\xF3rico 10%" },
  ],
...
function E({ source: t, spreadPct: s, gap: e }) {
  let o = 0.6;
  return (
    t === "leitura" && (o = 0.75),
    t === "memoria" &&
      (o = 0.75 - 0.25 * Math.max(0, Math.min(1, (a(s) - 10) / 30))),
    e <= 0.3 && (o = Math.min(o, 0.5)),
    o
  );
}
...
export function phDoseSuggestion({
  ...
  const g = phSlopeFromReadings([...(Array.isArray(r) ? r : []), { ph: u }], p),
    d = phPriorFor(t, l, n),
    f = g ?? d.slope,
    h = g !== null ? "leitura" : d.source,
    m = u - i;
  if (m <= y(t)) return { doseMl: 0, slope: f, source: h };
  const P = E({ source: h, spreadPct: d.spreadPct, gap: m }),
    S = M(P * m * f * p);
  return { doseMl: Math.max(0, S), slope: f, source: h };
}
export function spargeDoseFromWaterSlope({
  ...
  return r <= 0 || l <= 0 || n <= A
    ? { doseMl: 0 }
    : { doseMl: Math.max(0, M(0.85 * n * r * l)) };
}
export function updatePhMemory(
  t,
  { kind: s, slope: e, acidId: o = DEFAULT_PH_ACID, at: r } = {},
) {
  const l = a(e);
  if (!s || l <= 0) return t || null;
  const n = t && t.acidId === o ? { ...t } : { acidId: o };
  n.acidId = o;
  const c = n[s];
  if (!c || !(a(c.slope) > 0))
    return (
  ...
  const i = 0.65 * l + 0.35 * a(c.slope),
    p = (Math.abs(l - a(c.slope)) / i) * 100;
  return (
    (n[s] = {
      slope: i,
      samples: a(c.samples, 0) + 1,
      spreadPct: Math.min(100, 0.5 * p + 0.5 * a(c.spreadPct)),
      lastAt: String(r || new Date().toISOString()),
    }),
    n
  );
  ...
  return (
    PH_STAGES.forEach((o) => {
      const r = s[o] && typeof s[o] == "object" ? s[o] : {},
        l = (Array.isArray(r.readings) ? r.readings : [])
          .map((n) => ({
            ph: a(n?.ph),
