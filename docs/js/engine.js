...
export function effectiveWriFactor(t = {}) {
    ...
    return Number.isFinite(e) && e > .5 && e < 2 ? e : 1.04
}
export const CALIBRATION_PROFILE = {
    ...
    EMPTY_BASE_WATER_PROFILE = WATER_IONS.reduce((t, e) => ({ ...t,
        [e.key]: 0
    }), {}),
...
export function parseUserNumber(t) {
    ...
    const r = String(t).trim().replace(/\s+/g, "").replace(",", ".").match(/-?\d+(?:\.\d+)?/);
...
export function abvBrewfather(t, e) {
    const r = n(t, 1),
        o = n(e, 1),
        a = 1.775 - r;
    return r <= 1 || o <= 0 || a <= 0 ? 0 : Math.max(0, 76.08 * (r - o) / a * (o / .794))
}
...
export function slugify(t) {
    return String(t || "brassagem").normalize("NFD").replace(/[̀-ͯ]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || "brassagem"
}
...
export function formatInputValue(t) {
    ...
    const e = Number(t);
    return Number.isFinite(e) ? String(Math.round(e * 1e6) / 1e6).replace(".", ",") : String(t).replace(".", ",")
}
export function formatVolume(t, e = 1) {
    const r = Number(t);
    return Number.isFinite(r) ? Math.abs(r) >= 1e3 ? `${fmt(r/100,1)} hL` : `${fmt(r,e)} L` : "-"
}
...
export function formatMaltMass(t) {
    ...
    const r = Math.abs(e);
    return r < 1 ? `${fmt(e*1e3,1)} g` : r < 100 ? `${fmt(e,2)} kg` : `${fmt(e,1)} kg`
}
...
export function formatIngredientAmount(t, e) {
    const r = Number(t);
    return Number.isFinite(r) ? e === "kg" ? formatMaltMass(r) : e === "g" ? formatHopMass(r) : e === "mg" ? Math.abs(r) < 1e3 ? `${fmt(r,0)} mg` : formatHopMass(r / 1e3) : e === "L" ? formatVolume(r, 2) : e === "mL" ? Math.abs(r) < 1e3 ? `${fmt(r,1)} mL` : formatVolume(r / 1e3, 2) : `${fmt(r,1)} ${e||""}`.trim() : "-"
}
export function formatYeastAmount(t, e) {
    const r = Number(t);
    if (!Number.isFinite(r) || r <= 0) return "-";
    const o = fmtClean(r, ["pacote", "sach\xEA", "un."].includes(e) ? 2 : 1);
    return e === "pacote" ? `${o} ${Math.abs(r-1)<.005?"pacote":"pacotes"}` : e === "sach\xEA" ? `${o} ${Math.abs(r-1)<.005?"sach\xEA":"sach\xEAs"}` : `${o} ${e||"un."}`
}
...
export function sanitizeProductionProfile(t = {}) {
    const e = {};
    return PROFILE_KEYS.forEach(r => {
        const o = DEFAULT_PROFILE[r],
            a = n(t[r], o);
        e[r] = Number.isFinite(a) && a >= 0 ? a : o
    }), Object.prototype.hasOwnProperty.call(t, "baseWaterProfile") && (e.baseWaterProfile = sanitizeBaseWaterProfile(t.baseWaterProfile)), e.trubLossPct > 1 && (e.trubLossPct = e.trubLossPct / 100), e.wriFactor = effectiveWriFactor(e), e
}
...
export function mashEfficiencyFromEquipment(t, e, r) {
    const o = Math.max(.01, n(e)),
        a = o + Math.max(0, n(r));
    return round(n(t, DEFAULT_PROFILE.mashEfficiencyPct) * a / o, 1)
}
export function syncTrubLoss(t) {
    const e = Math.max(0, n(t.targetVolumeL, 20));
    Number.isFinite(Number(t.trubLossPct)) || (t.trubLossPct = e ? round(n(t.trubLossL, e * .15) / e, 4) : .15), t.trubLossEdited || (t.trubLossL = round(e * n(t.trubLossPct, .15), 2))
}
export function importedProductionProfile(t = {}) {
    const e = (t.fermentables || []).filter(u => u.use === "Mostura").reduce((u, c) => u + n(c.amountKg), 0),
        ...
        s = evaporationLhFromPct(n(t.evaporationPct, DEFAULT_PROFILE.evaporationPct), i, t.boilTimeMin),
        m = n(t.evaporationLh, s);
    return {
        targetVolumeL: o,
        mashEfficiencyPct: n(t.mashEfficiencyPct, n(t.efficiencyPct, DEFAULT_PROFILE.mashEfficiencyPct)),
        evaporationPct: Number.isFinite(Number(t.evaporationPct)) ? n(t.evaporationPct) : evaporationPctFromLh(m, i, t.boilTimeMin),
        trubLossPct: o ? round(a / o, 4) : DEFAULT_PROFILE.trubLossPct,
        trubLossL: round(a, 2),
        trubLossEdited: !1,
        grainAbsorptionLkg: DEFAULT_PROFILE.grainAbsorptionLkg,
        waterToGrainRatioLkg: e && r ? round(r / e, 2) : DEFAULT_PROFILE.waterToGrainRatioLkg,
        baseWaterProfile: sanitizeBaseWaterProfile(t.baseWaterProfile, DEFAULT_BASE_WATER_PROFILE)
    }
}
export function defaultProperties(t, e = {}) {
    const r = { ...DEFAULT_PROFILE,
            ...e
        },
        o = round(n(r.targetVolumeL, 20) * n(r.trubLossPct, .15), 2),
    ...
    return {
        ...
        evaporationLh: round(evaporationLhFromPct(r.evaporationPct, a, t && t.boilTimeMin), 2),
        ...
        heatingRateCMin: Math.min(10, Math.max(0, n(r.heatingRateCMin, DEFAULT_PROFILE.heatingRateCMin))),
        baseWaterProfile: e.baseWaterProfile && !isEmptyBaseWaterProfile(e.baseWaterProfile) ? sanitizeBaseWaterProfile(e.baseWaterProfile, DEFAULT_BASE_WATER_PROFILE) : sanitizeBaseWaterProfile(t.baseWaterProfile, DEFAULT_BASE_WATER_PROFILE),
        mashWaterUsedL: "",
        showWaterSalts: !1,
...
export function emptyWaterSalts() {
    return DEFAULT_WATER_SALTS.map(t => ({ ...t,
        sourceTotalG: 0,
...
export function saltFormulaFromName(t) {
    ...
    return /cacl/.test(e) || e.includes("calcium chloride") ? "CaCl2" : /caso/.test(e) || e.includes("gypsum") ? "CaSO4" : /mgso/.test(e) || e.includes("epsom") ? "MgSO4" : ""
}
export function waterSaltsFromRecipe(t = {}) {
    const e = emptyWaterSalts(),
        r = n(t.saltReferenceWaterL, t.batchVolumeL || 0);
    return (t.salts || []).forEach(o => {
...
export function ensureBaseWaterProfile(t) {
    return t.baseWaterProfile = sanitizeBaseWaterProfile(t.baseWaterProfile), t.baseWaterProfile
}
...
export function adjustedWaterProfile(t, e) {
    ...
    const s = WATER_IONS.reduce((m, u) => (m[u.key] = round(n(r[u.key]) + n(i[u.key]), 0), m), {});
    return {
        base: r,
        additions: i,
        adjusted: s
...
export function waterProfileSummary(t = {}) {
    return WATER_IONS.map(e => `${e.plainLabel} ${formatIonPpm(t.adjusted?.[e.key])}`).join(" \xB7 ")
}
export function createRecipeSession(t, e = "", r = {}) {
    const o = clonePlain(t);
    return o.sourceUrl = e, {
        recipe: o,
...
export function calculate(t) {
    const e = t.recipe,
        r = t.properties;
    syncTrubLoss(r);
    const o = Math.max(.1, n(e.batchVolumeL, r.targetVolumeL)),
        a = n(r.targetVolumeL, o) / o,
        i = Math.max(.1, n(e.mashEfficiencyPct, e.efficiencyPct || 70)) / Math.max(.1, n(r.mashEfficiencyPct, 65)),
        s = e.fermentables.map(h => ({ ...h,
            amountKg: round(n(h.amountKg) * a * (h.use === "Mostura" ? i : 1), 3)
        })),
        m = e.hops.map(h => ({ ...h,
            amountG: round(n(h.amountG) * a, 1)
        })),
        u = (e.yeasts || []).map(h => ({ ...h,
        ...
        d = s.filter(h => h.use === "Mostura").reduce((h, R) => h + n(R.amountKg), 0),
        p = n(r.targetVolumeL) + n(r.trubLossL),
        l = hotPostBoilVolume(r.targetVolumeL, r.trubLossL);
    r.evaporationLh = round(evaporationLhFromPct(r.evaporationPct, l, e.boilTimeMin), 2);
    const b = r.evaporationLh * e.boilTimeMin / 60,
        ...
        J = O.extraBoilMin ? calculateIbu(v.map(h => ({ ...h,
            timeMin: ["Fervura", "First wort"].includes(h.use) ? h.timeMin + O.extraBoilMin : h.timeMin
        })), F, T, r.targetVolumeL) : H,
        G = { ...O,
            estimatedIbu: Math.round(J)
        },
        _ = G.status === "ready" ? G.targetVolumeL : C,
        X = analyze(s, P, t.measurements, r, e, _, B, C),
        Q = v.filter(h => h.actualAlphaAcidPct === 0 || h.actualAlphaAcidPct === "0").map(h => `${h.name}: alfa \xE1cido real zero n\xE3o \xE9 v\xE1lido; dose mant\xE9m o lote planejado.`);
    return {
        recipe: e,
        props: r,
        ...
        expected: {
            ...
            fermenterVolume: round(Math.max(0, _ * .96 - r.trubLossL), 2)
        },
        analysis: X,
        warnings: Q
    }
...
export function moreyEbc(t, e) {
    const r = Math.max(.1, lToGal(n(e, 20))),
        o = (t || []).reduce((a, i) => a + kgToLb(n(i.amountKg)) * n(i.colorLovibond), 0) / r;
    return o <= 0 ? 0 : round(1.4922 * Math.pow(o, .6859) * 1.97, 1)
}
export function gravityEstimate(t, e) {
    const r = Math.max(.1, n(e.targetVolumeL, 20));
    let o = 0,
        a = 0,
        i = 0;
    return t.forEach(s => {
        ...
        o += u, s.use === "Mostura" && (a += u), s.use !== "Fermenta\xE7\xE3o" && (i += u)
    }), {
        points: o,
        preBoilPoints: a,
        preFermentPoints: i,
...
export function plannedPreBoilSg(t, e, r) {
    ...
    return 1 + o * s * n(r.hotPostBoil) / Math.max(.1, n(r.preBoil)) / 1e3
}
export function correction(t, e, r, o, a = 0, i = 1.04) {
    ...
    const c = m * u / e,
        d = c - m,
        p = Math.abs(d) <= .05 ? "Sem corre\xE7\xE3o" : d > 0 ? "Adicionar \xE1gua" : o === "post" ? "Ferver mais" : "Evaporar",
    ...
    return {
        ...
        estimatedIbu: 0,
        warning: p === "Ferver mais" ? "Fervura extra pode aumentar o IBU. Reavalie amargor antes de confirmar." : ""
    }
}
export function negligibleCorrection(t = {}) {
    if (t.status !== "ready" || t.action === "Sem corre\xE7\xE3o") return !1;
    const e = Math.max(.1, n(t.targetVolumeL) * .01);
...
export function correctionCheckReading(t, e, r, o, a = 1.04) {
    ...
    const i = t ? .correctionChecks ? .[e],
        s = normalizeReading({
            volumeL: r.targetVolumeL,
            wri: i ? .wri ? ? ""
        }, a);
    return s.realPlato ? { ...s,
        volumeL: r.targetVolumeL,
        expectedPlato: o
    } : null
}
...
export function correctionActionText(t) {
    const e = String(t || "");
    return /^Adicionar/.test(e) ? "Adicionar mais \xE1gua" : /^(Evaporar|Ferver)/.test(e) ? "Ferver mais" : e
}
export function correctionCheckResult(t, e, r, o, a = 0, i = 1.04) {
    const s = normalizeReading({
            volumeL: t.targetVolumeL,
            wri: e ? .wri ? ? ""
        }, i),
    ...
    return {
        status: "adjust",
        title: "Ajuste fino",
        summary: f === "Ferver mais" ? `${f}: reduzir ${g}${L}; conferir novamente` : `${f}: ${g}; conferir novamente`,
        detail: b
    }
}
export function correctionSummary(t) {
    return t.status === "pending" ? "Aguardando leitura" : t.action === "Sem corre\xE7\xE3o" ? "Sem corre\xE7\xE3o" : `${t.action} ${formatVolume(Math.abs(t.deltaL),2)}`
}
export function calculateIbu(t, e, r, o) {
    ...
    return t.reduce((s, m) => {
        ...
        const u = ["Hopstand", "Whirlpool"].includes(m.use) ? .3 : m.use === "First wort" ? 1.1 : 1,
            c = 1.65 * Math.pow(125e-6, a - 1) * (1 - Math.exp(-.04 * n(m.timeMin))) / 4.15;
...
export function analyze(t, e, r, o, a, i, s = e.preBoil, m = i) {
    const u = effectiveWriFactor(o),
        ...
        E = t.filter(S => S.use === "Mostura").reduce((S, T) => S + kgToLb(n(T.amountKg)) * n(T.ppg, 36), 0),
        P = r.cold || {},
        y = n(P.fermenterVolumeL, Math.max(0, i * .96 - o.trubLossL)),
        F = f && g ? Math.max(0, (f - g) / Math.max(.01, a.boilTimeMin / 60)) : 0;
    return {
        ...
        trubLossL: round(n(P.trubVolumeL, Math.max(0, i * .96 - y)), 2)
    }
}
export function isBoilAddition(t = {}) {
    return ["First wort", "Fervura", "Hopstand", "Whirlpool", "Dry hop"].includes(t.use)
}
export function additionOrder(t = {}) {
    return t.use === "First wort" ? 1e4 + n(t.timeMin) : t.use === "Fervura" ? 5e3 + n(t.timeMin) : t.use === "Hopstand" || t.use === "Whirlpool" ? 1e3 + n(t.timeMin) : t.use === "Dry hop" ? -100 : n(t.timeMin)
}
export function boilAdditionRows(t, e, r, o) {
    const a = n(o, 1);
    return [...e.filter(isBoilAddition).map(i => ({ ...i,
        kind: "hop",
        type: "L\xFApulo",
        amount: i.amountG,
        unit: "g",
        order: additionOrder(i)
    })), ...t.filter(i => i.use === "Fervura").map(i => ({
        id: i.id,
        kind: "fermentable",
        name: i.name,
        type: i.type,
    ...
    })), ...(r || []).filter(isBoilAddition).map(i => ({ ...i,
        kind: "misc",
...
export function mashAdditionRows(t, e) {
    return [...t.filter(r => r.use === "Mostura").map(r => ({ ...r,
        kind: "hop",
        type: "L\xFApulo",
        amount: r.amountG,
        unit: "g",
        moment: "Mash hopping",
        order: n(r.timeMin)
    })), ...(e || []).filter(r => r.use === "Mostura").map(r => ({ ...r,
        kind: "misc",
        moment: r.type,
        order: n(r.timeMin)
...
export function groupBySchedule(t = []) {
    ...
    return t.forEach(r => {
        ...
        i || (i = {
            ...
            timeLabel: !o && Number.isFinite(Number(r.timeMin)) ? `${fmt(r.timeMin,0)} min` : "",
            useLabel: r.use || "",
            items: []
        }, e.push(i)), i.items.push(r)
    }), e.sort((r, o) => o.order - r.order)
}
export function additionScheduleLabel(t) {
    return /dry hop/i.test(t.use) ? "Dry hop" : Number.isFinite(Number(t.timeMin)) ? `${fmt(t.timeMin,0)} min \xB7 ${t.use}` : t.use
}
export function mashStepName(t) {
    return String(t || "").trim().toLowerCase() === "temperature" ? "Temperatura" : t
}
export function effectiveMashSteps(t = []) {
    return (t || []).filter(e => {
        ...
        return !(/mash\s*out|mashout|sa[ií]da|finaliza/.test(`${o} ${a}`) || /ramp|subida|aquec|heat|rise|increase/.test(o))
    })
}
...
export function mashTimerStepInfo(t = {}) {
    const e = mashStepName(t.name || "Mostura"),
        r = Number.isFinite(Number(t.temperatureC)) ? `${fmt(t.temperatureC,1)}\xB0C` : e;
    return {
        label: e,
        target: r,
        detail: r,
        info: `${fmt(t.timeMin,0)} min`
    }
}
export const DEFAULT_HEATING_RATE_C_MIN = 1.5;
export function effectiveHeatingRateCMin(t = {}, e = {}) {
    return Math.min(10, Math.max(0, n(e.heatingRateCMin, n(t.heatingRateCMin, DEFAULT_HEATING_RATE_C_MIN))))
}
export function mashTimerItems(t = [], e = 0) {
    ...
    const i = a.reduce((u, c) => u + (c.kind === "step" ? n(c.step.timeMin) : c.durationMin), 0),
        s = [];
    let m = i;
    return a.forEach((u, c) => {
        ...
        if (u.kind === "heat") {
            ...
            s.push({
                ...
                eventActionLabel: f ? L ? `Aquecer at\xE9 ${f.target}` : `Confirmar ${f.label}` : "Finalizar mostura",
                eventSummaryLabel: f ? L ? `Aquecer at\xE9 ${f.target}` : `Confirmar ${f.label}` : "Finalizar mostura",
...
export function boilAdditionTimerTime(t, e) {
    return ["First wort", "Fervura"].includes(t.use) ? Number.isFinite(Number(t.timeMin)) ? Math.max(0, Math.min(e, n(t.timeMin))) : t.use === "First wort" ? e : Math.min(10, e) : null
}
export function hopstandAdditionTimerTime(t) {
    return ["Hopstand", "Whirlpool"].includes(t.use) ? Number.isFinite(Number(t.timeMin)) ? Math.max(0, n(t.timeMin)) : 0 : null
}
...
export function boilTimerItems(t) {
    const e = n(t.recipe.boilTimeMin, 60),
        ...
        i = Array.from(o.keys()).map(Number).sort((l, b) => b - l),
        s = e > POST_READING_OFFSET_MIN ? POST_READING_OFFSET_MIN : null,
        m = uniqueTimerTimes([...i, ...s === null ? [] : [s], 0]).sort((l, b) => b - l),
        u = [];
    let c = e;
    m.forEach((l, b) => {
        ...
        u.push({
            id: `boil-${b}-${round(l,2)}`,
            phase: "boil",
            label: g ? `Adi\xE7\xE3o de ${x}` : M && l > 0 ? "Leitura p\xF3s-fervura" : "Fim da fervura",
            detail: g ? additionCountLabel(f.length) : M && l > 0 ? "leia volume e WRI ainda fervendo" : "fim da fervura",
            durationMin: L,
            boilRemainingMin: c,
            ...
            eventCount: f.length,
            eventActionLabel: g ? `Confirmar adi\xE7\xE3o de ${x}` : M && l > 0 ? "Fazer a leitura p\xF3s-fervura" : a.size ? "Iniciar hopstand" : "Finalizar fervura",
            eventSummaryLabel: g ? `Pr\xF3x. ${x}` : M && l > 0 ? `Leitura ${x}` : "Fim",
            additions: f.map(timerAdditionSummary),
            withReading: M,
            meta: g ? `${additionCountLabel(f.length)} em ${x}` : `${fmt(L,0)} min at\xE9 o fim da fervura`
        }), c = l
    });
    const d = Array.from(a.keys()).map(Number).sort((l, b) => b - l);
    let p = d[0] || 0;
...
export function expectedFermentationTemperature(t = [], e = 0) {
    ...
    const a = [...t].reverse().find(i => Number.isFinite(Number(i.temperatureC)));
...
export function fermentationDayFromDatetime(t = {}, e = null, r = 0) {
    ...
    return e && o ? Math.floor(Math.max(0, (o.getTime() - e.getTime()) / 864e5)) : t.day === "" ? r + 1 : Math.floor(Math.max(0, n(t.day, r + 1)))
}
...
export function fermentedRefractometerPlato(t, e, r = 1.04) {
    ...
    const o = n(r) > 0 ? n(r) : 1.04,
        a = 1 - .002349 * n(t) + .006276 * (n(e) / o);
    return round(Math.max(0, sgToPlato(a)), 1)
}
export function sanitizeFermentationTracking(t = {}) {
    return {
        readings: (Array.isArray(t ? .readings) ? t.readings : []).map(sanitizeFermentationReading).filter(Boolean)
    }
}
export function sanitizeFermentationReading(t = {}, e = 0) {
    return isPlainObject(t) ? {
        ...
        temperatureC: t.temperatureC === "" ? "" : Math.max(0, n(t.temperatureC)),
...
export function fermentationInitialReading(t, e) {
    ...
    if (o.realPlato) return fermentationInitialReadingValue("OG real \xB7 confer\xEAncia p\xF3s-corre\xE7\xE3o", o.realPlato, o.wri, r);
    const a = normalizeReading(t.measurements ? .postBoil || {}, r);
    return a.realPlato ? fermentationInitialReadingValue("OG real \xB7 leitura p\xF3s-fervura", a.realPlato, a.wri, r) : fermentationInitialReadingValue("OG planejada", e.ogPlato, e.ogPlato * r, r)
}
export function fermentationTrackingState(t, e) {
    const r = fermentationInitialReading(t, e);
    r.expectedTemperatureC = expectedFermentationTemperature(e.recipe.fermentation, 0);
    const o = {
        plato: round(Math.max(0, sgToPlato(e.fg)), 1),
        sg: e.fg
    };
    ...
    const a = t.fermentationTracking.readings,
        ...
        m = a.map((u, c) => {
            ...
            return {
                ...
                day: l,
                datetime: u.datetime || "",
                expectedTemperatureC: expectedFermentationTemperature(e.recipe.fermentation, l),
...
export function fermentationChartModel(t, e) {
    const r = fermentationTrackingState(t, e),
        ...
        a = r.readings.filter(c => c.temperatureC !== "").map(c => ({
            day: c.day,
            value: c.temperatureC
        })),
        i = [{
            day: 0,
            value: r.initial.plato,
            label: r.initial.source
        }, ...r.readings.filter(c => c.realPlato !== "").map(c => ({
            day: c.day,
            value: c.realPlato,
            label: "Leitura"
        }))],
        ...
        u = Math.max(1, ...o.map(c => c.day).filter(Number.isFinite), ...a.map(c => c.day).filter(Number.isFinite), ...i.map(c => c.day).filter(Number.isFinite));
    return {
        maxDay: Math.ceil(u),
        tempPoints: o,
        realTempPoints: a,
        ...
        extractRange: paddedRange(m, .5, 4)
    }
}
...
export function finalParameterCode(t) {
    const e = t.analysis || {},
        ...
        i = o ? a / o : n(r.trubLossPct, .15),
        s = pickParameterValue(e.mashEfficiencyPct, r.mashEfficiencyPct),
        m = o + Math.max(0, n(a)),
        u = m ? s * o / m : s;
    return [`vol=${fmt(o,1)}`, `equip=${fmt(u,1)}`, `evap=${fmt(pickParameterValue(e.evaporationPct,r.evaporationPct),1)}`, `trub=${fmt(i*100,1)}%`, `abs=${fmt(pickParameterValue(e.grainAbsorptionLkg,r.grainAbsorptionLkg),2)}`, `agua=${fmt(r.waterToGrainRatioLkg,2)}`].join("; ")
}
...
export function hasCompleteParameterValues(t = {}) {
    return ["V", "EV", "TR", "ABS", "RAM"].every(e => Number.isFinite(t[e])) && (Number.isFinite(t.EM) || Number.isFinite(t.EE)) && t.V > 0 && (t.EM > 0 || t.EE > 0) && t.EV >= 0 && t.TR >= 0 && t.ABS > 0 && t.RAM > 0
}
export function applyProductionParameterValues(t, e) {
    Number.isFinite(e.V) && (t.targetVolumeL = e.V), Number.isFinite(e.EV) && (t.evaporationPct = e.EV), Number.isFinite(e.ABS) && (t.grainAbsorptionLkg = e.ABS), Number.isFinite(e.RAM) && (t.waterToGrainRatioLkg = e.RAM), Number.isFinite(e.TR) && (t.trubLossPct = e.TR > 1 ? e.TR / 100 : e.TR, t.trubLossL = round(n(t.targetVolumeL, 20) * t.trubLossPct, 2), t.trubLossEdited = !1), Number.isFinite(e.EE) ? t.mashEfficiencyPct = mashEfficiencyFromEquipment(e.EE, t.targetVolumeL, t.trubLossL) : Number.isFinite(e.EM) && (t.mashEfficiencyPct = e.EM)
}
...
export function parseReadableParameterCode(t) {
    ...
    return String(t || "").split(/[;\n]+/).forEach(r => {
        ...
        const a = normalizeParameterKey(o[1]),
            i = o[2],
            s = parseUserNumber(i);
        Number.isFinite(s) && (["v", "vol", "volume", "litros"].includes(a) && (e.V = s), ["em", "mostura", "eficienciamostura"].includes(a) && (e.EM = s), ["ee", "ef", "equip", "equipamento", "eficiencia", "eficienciaequipamento", "eficienciadoequipamento"].includes(a) && (e.EE = s), ["ev", "evap", "evaporacao"].includes(a) && (e.EV = s), ["tr", "trub", "perdatrub"].includes(a) && (e.TR = i.includes("%") || s > 1 ? s / 100 : s), ["abs", "absorcao", "absorcaoporgrao"].includes(a) && (e.ABS = s), ["agua", "aguamalte", "ram", "relacao", "relacaoaguamalte"].includes(a) && (e.RAM = s))
    }), e
}
export function parseShortParameterCode(t) {
    const e = String(t || "").toUpperCase().replace(/\s+/g, "");
...
export function parsePipeParameterCode(t) {
    ...
    return e.split("|").forEach(o => {
        const [a, ...i] = o.split("="), s = String(a || "").trim().toUpperCase();
...
export function parseParameterText(t) {
    ...
    return e && [parseReadableParameterCode(e), parseShortParameterCode(e), parsePipeParameterCode(e)].find(hasCompleteParameterValues) || null
}
export function originalWaterPlan(t) {
    const e = (t.fermentables || []).filter(u => u.use === "Mostura").reduce((u, c) => u + n(c.amountKg), 0),
        o = (t.mash || []).reduce((u, c) => u + n(c.waterVolumeL), 0) || e * DEFAULT_PROFILE.waterToGrainRatioLkg,
